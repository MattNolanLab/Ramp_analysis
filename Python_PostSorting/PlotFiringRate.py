...
def plot_rewarded_p_firing_rate(spike_data, prm):
    ...
    for cluster in range(len(spike_data)):
        ...
        speed_histogram = plt.figure(figsize=(4,3))
        ax = speed_histogram.add_subplot(1, 1, 1)  # specify (nrows, ncols, axnum)
        ax.plot(position_array,rates, '-', color='Black', alpha = 0.8)
        ax.fill_between(position_array, rates-sd_rates,rates+sd_rates, facecolor = 'Black', alpha = 0.2)

        ax.plot(position_array,rates_nb, '-', color='Blue', alpha = 0.8)
        ax.fill_between(position_array, rates_nb-sd_rates_nb,rates_nb+sd_rates_nb, facecolor = 'Blue', alpha = 0.2)
...
def extract_time_binned_firing_rate_failed(spike_data, prm):
    spike_data["Rates_averaged_failed_b"] = ""
    spike_data["Rates_averaged_failed_nb"] = ""
    spike_data["Rates_averaged_failed_p"] = ""
    spike_data["Rates_sd_failed_b"] = ""
    spike_data["Rates_sd_failed_nb"] = ""
    spike_data["Rates_sd_failed_p"] = ""
    spike_data["Firing_rate_failed_b"] = ""
    spike_data["Firing_rate_failed_nb"] = ""
    spike_data["Firing_rate_failed_p"] = ""
    ...
    for cluster in range(len(spike_data)):
        ...
        trials=np.array(spike_data.iloc[cluster].spikes_in_time_failed[3], dtype= np.int32)
        ...
        x = np.nanmean(data_b, axis=1)
        x_sd = np.nanstd(data_b, axis=1)
        spike_data.at[cluster, 'Rates_averaged_failed_b'] = list(x)# add data to dataframe
        spike_data.at[cluster, 'Rates_sd_failed_b'] = list(x_sd)
        spike_data.at[cluster, 'Firing_rate_failed_b'] = data_b
        ...
        x = np.nanmean(data_nb, axis=1)
        x_sd = np.nanstd(data_nb, axis=1)
        spike_data.at[cluster, 'Rates_averaged_failed_nb'] = list(x)
        spike_data.at[cluster, 'Rates_sd_failed_nb'] = list(x_sd)
        spike_data.at[cluster, 'Firing_rate_failed_nb'] = data_nb
        ...
        x = np.nanmean(data_p, axis=1)
        x_sd = np.nanstd(data_p, axis=1)
        spike_data.at[cluster, 'Rates_averaged_failed_p'] = list(x)
        spike_data.at[cluster, 'Rates_sd_failed_p'] = list(x_sd)
        spike_data.at[cluster, 'Firing_rate_failed_p'] = data_p
