...
def calculate_progression(spike_data):
    print('calculating graduation day')
    select_df = spike_data[["Mouse", "Day_numeric", "probe_trials", "Reward_Rate"]]
    ...
    for m, mcount in enumerate(unique_mice):
        ...
        write_to_csv(np.array(df), mcount)
...
## Save to .csv file
def write_to_csv(csvData,m):
    with open('/Users/sarahtennant/Work/Analysis/Ramp_analysis/data/graduation-' + str(m) + '.csv', 'w') as csvFile:
