...
def plot_spikes_on_track_tracks(recording_folder,spike_data, prm, prefix):
    print('plotting spike rasters...')
    save_path = recording_folder + '/Figures/spike_trajectories_lengths'
    if os.path.exists(save_path) is False:
        os.makedirs(save_path)
    ...
    for cluster in range(len(spike_data)):
        cluster_index = spike_data.cluster_id.values[cluster] - 1
        x_max = spike_data.loc[cluster, 'max_trial_number']+1
        spikes_on_track = plt.figure(figsize=(6,3.5))
        ax = spikes_on_track.add_subplot(1, 1, 1)  # specify (nrows, ncols, axnum)
        beaconed_position_cm, nonbeaconed_position_cm, probe_position_cm, beaconed_trial_number, nonbeaconed_trial_number, probe_trial_number = Python_PostSorting.ExtractFiringData.split_firing_by_trial_type(spike_data, cluster)

        ax.plot(beaconed_position_cm/2, beaconed_trial_number, '|', color='Black', markersize=2)
        ax.plot(nonbeaconed_position_cm/2, nonbeaconed_trial_number, '|', color='Red', markersize=2)
        ax.plot(probe_position_cm, probe_trial_number, '|', color='Blue', markersize=2)
...
def plot_spikes_on_track_per_trialtype2(recording_folder,spike_data, prm, prefix='_movement'):
    print('plotting spike rasters per trial type...')
    save_path = recording_folder + '/Figures/spike_trajectories/per_trialtype'
    if os.path.exists(save_path) is False:
        os.makedirs(save_path)
    ...
    for cluster in range(len(spike_data)):
        ...
        spikes_on_track = plt.figure(figsize=(4,3))
        ax = spikes_on_track.add_subplot(1, 1, 1)  # specify (nrows, ncols, axnum)
        ax.plot(beaconed_position_cm, beaconed_trial_number, '|', color='Black', markersize=2.5)
        plt.ylabel('Spikes on trials', fontsize=14, labelpad = 10)
        plt.xlabel('Location (cm)', fontsize=14, labelpad = 10)
        plt.xlim(30,90)
        plt.ylim(0,80)
        ...
        #ax.set_xticklabels(['-30', '70', '170'])
        plt.tight_layout(pad=0.4, w_pad=0.5, h_pad=1.0)
        plt.savefig(save_path + '/' + spike_data.session_id[cluster] + '_track_firing_Cluster_' + str(cluster_index +1) + str(prefix) + '_beaconed_outbound.png', dpi=200)
        plt.close()

        spikes_on_track = plt.figure(figsize=(4,3))
        ax = spikes_on_track.add_subplot(1, 1, 1)  # specify (nrows, ncols, axnum)
        ax.plot(nonbeaconed_position_cm, nonbeaconed_trial_number, '|', color='red', markersize=3)
        ax.plot(beaconed_position_cm, beaconed_trial_number, '|', color='Black', markersize=2.5, alpha=.2)
        plt.ylabel('Spikes on trials', fontsize=16, labelpad = 10)
        plt.xlabel('Location (cm)', fontsize=16, labelpad = 10)
        plt.xlim(30,90)
        plt.ylim(0,80)
        ...
        #ax.set_xticklabels(['-30', '70', '170'])
        plt.tight_layout(pad=0.4, w_pad=0.5, h_pad=1.0)
        plt.savefig(save_path + '/' + spike_data.session_id[cluster] + '_track_firing_Cluster_' + str(cluster_index +1) + str(prefix) + '_nbeaconed_outbound.png', dpi=200)
        plt.close()

        spikes_on_track = plt.figure(figsize=(4,3))
        ax = spikes_on_track.add_subplot(1, 1, 1)  # specify (nrows, ncols, axnum)
        ax.plot(probe_position_cm, probe_trial_number, '|', color='Blue', markersize=3)
        ax.plot(beaconed_position_cm, beaconed_trial_number, '|', color='Black', markersize=2.5, alpha=.2)
        plt.ylabel('Spikes on trials', fontsize=16, labelpad = 10)
        plt.xlabel('Location (cm)', fontsize=16, labelpad = 10)
        plt.xlim(30,90)
        ...
        #ax.set_xticklabels(['-30', '70', '170'])
        plt.tight_layout(pad=0.4, w_pad=0.5, h_pad=1.0)
        plt.savefig(save_path + '/' + spike_data.session_id[cluster] + '_track_firing_Cluster_' + str(cluster_index +1) + str(prefix) + '_probe_outbound.png', dpi=200)
        plt.close()
        ...
        #ax.set_xticklabels(['-30', '70', '170'])
        plt.tight_layout(pad=0.4, w_pad=0.5, h_pad=1.0)
        plt.savefig(save_path + '/' + spike_data.session_id[cluster] + '_track_firing_Cluster_' + str(cluster_index +1) + str(prefix) + '_beaconed_homebound.png', dpi=200)
        plt.close()
        ...
        plt.ylim(0,80)
        plt.tight_layout(pad=0.4, w_pad=0.5, h_pad=1.0)
        plt.savefig(save_path + '/' + spike_data.session_id[cluster] + '_track_firing_Cluster_' + str(cluster_index +1) + str(prefix) + '_nbeaconed_homebound.png', dpi=200)
        plt.close()
        ...
        #ax.set_xticklabels(['-30', '70', '170'])
        plt.tight_layout(pad=0.4, w_pad=0.5, h_pad=1.0)
        plt.savefig(save_path + '/' + spike_data.session_id[cluster] + '_track_firing_Cluster_' + str(cluster_index +1) + str(prefix) + '_probe_homebound.png', dpi=200)
        plt.close()



def plot_spikes_on_track_per_trialtype3(recording_folder,spike_data, prm, prefix='_movement'):
    print('plotting spike rasters per trial type...')
    save_path = recording_folder + '/Figures/spike_trajectories/per_trialtype'
    if os.path.exists(save_path) is False:
        os.makedirs(save_path)
...
def plot_spikes_on_track_per_trialtype_example2(recording_folder,spike_data, prm, prefix='_movement'):
    print('plotting example spike rasters per trial type...')
    save_path = recording_folder + '/Figures/spike_trajectories_examples/per_trialtype'
    if os.path.exists(save_path) is False:
        os.makedirs(save_path)
    ...
    for cluster in range(len(spike_data)):
        ...
        plt.subplots_adjust(hspace = .35, wspace = .35,  bottom = 0.2, left = 0.12, right = 0.87, top = 0.92)
        #plt.tight_layout(pad=0.4, w_pad=0.5, h_pad=1.0)
        plt.savefig(save_path + '/' + spike_data.session_id[cluster] + '_track_firing_Cluster_' + str(cluster_index +1) + '_beaconed_example_outbound.png', dpi=200)
        plt.close()

        spikes_on_track = plt.figure(figsize=(4,3))
        ax = spikes_on_track.add_subplot(1, 1, 1)  # specify (nrows, ncols, axnum)
        ax.plot(nonbeaconed_position_cm, nonbeaconed_trial_number, '|', color='Red', markersize=2.5)
        ax.plot(probe_position_cm, probe_trial_number, '|', color='Blue', markersize=2.5)
        plt.ylabel('Spikes on trials', fontsize=14, labelpad = 10)
        plt.xlabel('Location (cm)', fontsize=14, labelpad = 10)
        plt.xlim(30,90)
        ax.yaxis.set_ticks_position('left')
        ...
        plt.locator_params(axis = 'y', nbins  = 4)
        plt.tight_layout(pad=0.4, w_pad=0.5, h_pad=1.0)
        plt.savefig(save_path + '/' + spike_data.session_id[cluster] + '_track_firing_Cluster_' + str(cluster_index +1) + '_nbeaconed_example_outbound.png', dpi=200)
        plt.close()
        ...
        plt.locator_params(axis = 'y', nbins  = 4)
        plt.tight_layout(pad=0.4, w_pad=0.5, h_pad=1.0)
        plt.savefig(save_path + '/' + spike_data.session_id[cluster] + '_track_firing_Cluster_' + str(cluster_index +1) + '_probe_example_outbound.png', dpi=200)
        plt.close()
        ...
        plt.subplots_adjust(hspace = .35, wspace = .35,  bottom = 0.2, left = 0.12, right = 0.87, top = 0.92)
        #plt.tight_layout(pad=0.4, w_pad=0.5, h_pad=1.0)
        plt.savefig(save_path + '/' + spike_data.session_id[cluster] + '_track_firing_Cluster_' + str(cluster_index +1) + '_beaconed_example_homebound.png', dpi=200)
        plt.close()
        ...
        plt.locator_params(axis = 'y', nbins  = 4)
        plt.tight_layout(pad=0.4, w_pad=0.5, h_pad=1.0)
        plt.savefig(save_path + '/' + spike_data.session_id[cluster] + '_track_firing_Cluster_' + str(cluster_index +1) + '_nbeaconed_example_homebound.png', dpi=200)
        plt.close()
        ...
        plt.locator_params(axis = 'y', nbins  = 4)
        plt.tight_layout(pad=0.4, w_pad=0.5, h_pad=1.0)
        plt.savefig(save_path + '/' + spike_data.session_id[cluster] + '_track_firing_Cluster_' + str(cluster_index +1) + '_probe_example_homebound.png', dpi=200)
        plt.close()
...
def plot_smoothed_firing_rate_maps_tracks(recording_folder, spike_data, prefix):
    print('I am plotting smoothed firing rate maps...')
    save_path = recording_folder + '/Figures/spike_rate_smoothed'
    if os.path.exists(save_path) is False:
        os.makedirs(save_path)
    for cluster in range(len(spike_data)):
        ...
        avg_beaconed_spike_rate, avg_nonbeaconed_spike_rate, avg_probe_spike_rate, sd = Python_PostSorting.ExtractFiringData.extract_smoothed_average_firing_rate_data_tracks(spike_data, cluster)

        avg_spikes_on_track = plt.figure(figsize=(6,3.5))
        ax = avg_spikes_on_track.add_subplot(1, 1, 1)  # specify (nrows, ncols, axnum)
        ax.plot(np.arange(0,100,0.5),avg_beaconed_spike_rate, '-', color='Black')
        hist4=(avg_beaconed_spike_rate+avg_nonbeaconed_spike_rate)/2
        ax.plot(np.arange(0,100,0.5),hist4, '-', color='Red')
        ax.plot(np.arange(0,200,1),avg_probe_spike_rate, '-', color='Blue')
        ax.locator_params(axis = 'x', nbins=3)
...
def plot_smoothed_firing_rate_maps_per_trialtype2(recording_folder, spike_data, prefix):
    print('I am plotting smoothed firing rate maps for trial types...')
    save_path = recording_folder + '/Figures/spike_rate_smoothed/per_trialtype'
    if os.path.exists(save_path) is False:
        os.makedirs(save_path)
    for cluster in range(len(spike_data)):
        ...
        plt.savefig(save_path + '/' + spike_data.session_id[cluster] + '_smoothed_rate_map_Cluster_' + str(cluster_index +1)+ '_beaconed_outbound.png', dpi=200)
        plt.close()
        ...
        Python_PostSorting.plot_utility.style_track_plot(ax, 200)
        plt.subplots_adjust(hspace = .35, wspace = .35,  bottom = 0.4, left = 0.2, right = 0.8, top = 0.92)
        plt.savefig(save_path + '/' + spike_data.session_id[cluster] + '_smoothed_rate_map_Cluster_' + str(cluster_index +1) + '_nbeaconed_outbound.png', dpi=200)
        plt.close()
        ...
        Python_PostSorting.plot_utility.style_track_plot(ax, 200)
        plt.subplots_adjust(hspace = .35, wspace = .35,  bottom = 0.4, left = 0.2, right = 0.8, top = 0.92)
        plt.savefig(save_path + '/' + spike_data.session_id[cluster] + '_smoothed_rate_map_Cluster_' + str(cluster_index +1) + '_probe_outbound.png', dpi=200)
        plt.close()
        ...
        Python_PostSorting.plot_utility.style_track_plot(ax, 200)
        plt.subplots_adjust(hspace = .35, wspace = .35,  bottom = 0.4, left = 0.2, right = 0.8, top = 0.92)
        plt.savefig(save_path + '/' + spike_data.session_id[cluster] + '_smoothed_rate_map_Cluster_' + str(cluster_index +1)+ '_beaconed_homebound.png', dpi=200)
        plt.close()
        ...
        Python_PostSorting.plot_utility.style_track_plot(ax, 200)
        plt.subplots_adjust(hspace = .35, wspace = .35,  bottom = 0.4, left = 0.2, right = 0.8, top = 0.92)
        plt.savefig(save_path + '/' + spike_data.session_id[cluster] + '_smoothed_rate_map_Cluster_' + str(cluster_index +1) + '_nbeaconed_homebound.png', dpi=200)
        plt.close()
        ...
        Python_PostSorting.plot_utility.style_track_plot(ax, 200)
        plt.subplots_adjust(hspace = .35, wspace = .35,  bottom = 0.4, left = 0.2, right = 0.8, top = 0.92)
        plt.savefig(save_path + '/' + spike_data.session_id[cluster] + '_smoothed_rate_map_Cluster_' + str(cluster_index +1) + '_probe_homebound.png', dpi=200)
        plt.close()
...
def plot_firing_rate_gc_maps(recording_folder, spike_data, prefix):
    ...
    for cluster_index in range(len(spike_data)):
        ...
        unsmooth_b = np.array(spike_data.at[cluster_index, 'firing_maps_b'])
        unsmooth_nb = np.array(spike_data.at[cluster_index, 'firing_maps_nb'])
        unsmooth_p = np.array(spike_data.at[cluster_index, 'firing_maps_p'])
        ...
        x_max = (np.nanmax(np.array(spike_data.at[cluster_index, 'firing_maps_b']))) +5
        plt.locator_params(axis = 'y', nbins  = 4)
        Python_PostSorting.plot_utility.style_vr_plot(ax, x_max)
        Python_PostSorting.plot_utility.style_track_plot(ax, 80)
        plt.tight_layout(pad=0.4, w_pad=0.5, h_pad=1.0)
...
def plot_combined_spike_raster_and_rate(recording_folder,spike_data,processed_position_data, prefix):
    ...
    for cluster in range(len(spike_data)):
        ...
        plt.xlabel('Location (cm)', fontsize=10, labelpad = 10)
        plt.xlim(0,200)
        Python_PostSorting.plot_utility.style_vr_plot(ax1, x_max)
        Python_PostSorting.plot_utility.style_track_plot(ax1, 200)
        plt.tight_layout(pad=0.4, w_pad=0.5, h_pad=1.0)
...
def plot_rewarded_spikes_on_track2(recording_folder,spike_data):
    print('plotting spike rasters for rewarded trials...')
    save_path = recording_folder + '/Figures/spike_trajectories_split_by_reward'
    if os.path.exists(save_path) is False:
        os.makedirs(save_path)
    ...
    for cluster in range(len(spike_data)):
        ...
        plt.locator_params(axis = 'y', nbins  = 4)
        plt.tight_layout(pad=0.4, w_pad=0.5, h_pad=1.0)
        plt.savefig(recording_folder + '/Figures/spike_trajectories_split_by_reward/' + spike_data.session_id[cluster] + '_track_firing_Cluster_' + str(cluster_index +1) + '_rewarded2.png', dpi=200)
        plt.close()
...
def plot_failed_spikes_on_track2(recording_folder,spike_data):
    print('plotting spike rasters for failed trials...')
    save_path = recording_folder + '/Figures/spike_trajectories_split_by_reward'
    if os.path.exists(save_path) is False:
        os.makedirs(save_path)
    ...
    for cluster in range(len(spike_data)):
        ...
        plt.locator_params(axis = 'y', nbins  = 4)
        plt.tight_layout(pad=0.4, w_pad=0.5, h_pad=1.0)
        plt.savefig(recording_folder + '/Figures/spike_trajectories_split_by_reward/' + spike_data.session_id[cluster] + '_track_firing_Cluster_' + str(cluster_index +1) + '_failed2.png', dpi=200)
        plt.close()
...
def plot_color_coded_instant_rates_by_trial(recording_folder, spike_data):
    #spike_data = add_instant_rates_regression_to_frame(spike_data)
    #print('I am plotting instant rate against location ...')
    save_path = recording_folder + '/Figures/InstantRates'
    if os.path.exists(save_path) is False:
    ...
    for cluster in range(len(spike_data)):
        cluster_index = spike_data.cluster_id.values[cluster] - 1
        #rates =  np.array(spike_data.at[cluster, 'spike_rate_in_time'])
        ...
        data_filtered = data[data[:,1] > 2400,:]
        rates = data_filtered[:,0]
        #speed = data_filtered[:,1]
        position = data_filtered[:,2]
        ...
        avg_spikes_on_track = plt.figure(figsize=(4,3))
        ax = avg_spikes_on_track.add_subplot(1, 1, 1)  # specify (nrows, ncols, axnum)
        plt.scatter(position, rates, s=area, c=trials)
        cbar = plt.colorbar()
        cbar.ax.tick_params(labelsize=14)
        #slope, intercept, r_value, p_value, std_err = stats.linregress(position,rates)
        #line = slope*position+intercept
        #ax.plot(position, line, '-', color = 'red', linewidth=2)
        ax.set_xlim(0)
        ax.set_ylim(0)
        plt.ylabel('Spike rate (hz)', fontsize=16, labelpad = 10)
        plt.xlabel('Location (cm)', fontsize=16, labelpad = 10)
        ...
        plt.tight_layout(pad=0.4, w_pad=0.5, h_pad=1.0)
        plt.savefig(save_path + '/' + spike_data.session_id[cluster] + '_rate_map_Cluster_' + str(cluster_index +1) + '_location' + '_trial_coded.png', dpi=200)
        plt.close()
