...
data <- spatial_firing %>%
  ...
  mutate(lmgroup = rep(c("Negative", "Unclassified", "Positive"))) %>%
  select(proportion,lmgroup) %>%
  group_by(lmgroup) %>%
  summarise(avg = mean(proportion), sd = sd(proportion)/6)
...
1. extract shuffled values into tibble
```{r}
...
neurons_same <- (posnb_position_cells + negnb_position_cells)/total*100
```
...
```{r}
sig_ramps <- nrow(subset(ramps, reward_interaction_id == "Significant"))/nrow(ramps)*100
nonsig_ramps <- nrow(subset(ramps, reward_interaction_id == "Not-Significant"))/nrow(ramps)*100
non_ramps <- nrow(subset(ramps, reward_interaction_id == "None" | reward_interaction_id == "NA"))/nrow(ramps)*100
