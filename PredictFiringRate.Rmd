...
```{r}
predict_reset <-subset(spatial_firing, reset_group == "Reset" & lm_group_b == "Positive" & lm_group_b == "Positive")
```


```{r}

slope_comparison <- as.tibble(cbind(cluster_id=predict_reset$cluster_id, 
                          group = predict_reset$reset_group,
                          intercept_o = as.numeric(predict_reset$asr_b_o_rewarded_fit_intercept),
                          slope_o = as.numeric(predict_reset$asr_b_o_rewarded_fit_slope),
                          session_id = predict_reset$session_id, 
                          firing_rate_in_time = predict_reset$spikes_in_time,
                          Rates_averaged = predict_reset$asr_b_rewarded))

slope_comparison_predict <- as.tibble(cbind(cluster_id=predict_reset$cluster_id, 
                          group = predict_reset$reset_group,
                          session_id = predict_reset$session_id, 
                          predicted_Rates_averaged = predict_reset$predict_firingrate))
```
...
ggsave(file = "plots/facet_predict_correct.png", width = 14, height = 9)
```
