...
posneg_diff_probe <-subset(posneg, track_category_p != "posneg")
negpos_diff_probe <-subset(negpos, track_category_p != "negpos")
...
```{r}
ggplot() + 
    geom_point(data=posneg_same_probe,aes(x = as.numeric(Day_numeric), y = as.numeric(hit_trials_number), color=factor(unlist(lm_group_b))), shape=1, colour = "chartreuse3") +
    geom_point(data=posneg_diff_probe,aes(x = as.numeric(Day_numeric), y = as.numeric(hit_trials_number), color=factor(unlist(lm_group_b))), colour = "chartreuse3") +
    geom_point(data=negpos_same_probe,aes(x = as.numeric(Day_numeric), y = as.numeric(hit_trials_number), color=factor(unlist(lm_group_b))), shape=1, colour = "violetred2") +
    geom_point(data=negpos_diff_probe,aes(x = as.numeric(Day_numeric), y = as.numeric(hit_trials_number), color=factor(unlist(lm_group_b))), colour = "violetred2") +    
  #coord_cartesian(ylim = c(-5,5), xlim = c(-5,5)) +
    #geom_abline(intercept = 0, slope = 1, colour = "grey", linetype = "dashed") +
    xlab("Beaconed offset") +
    ylab("Probe offset") +
...
pospos_r_un <-subset(pospos_r, track_category_p == "posnon")
negneg_r_un <-subset(negneg_r, track_category_p == "negnon")
