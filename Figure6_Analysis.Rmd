...
```{r}
spatial_firing <- spatial_firing %>%
  mutate(track_reset_group_p = pmap(list(lm_group_p, lm_group_p_h, reset_group_p), mark_track_category_reset))
...
1. extract only neurons that are driven by path integration (as determined by CueAnalysis.0100.Rmd - i.e. Figure 4)
```{r}
...
```{r}
pi_data <- subset(pi_data, track_catagory_p == "pospos" | track_catagory_p == "negneg")
```
...
```{r}
pi_data_for_regression <- pi_data %>% 
...
Calculate r2 and p value for liner correlation of the above data
```{r}
pi_data_for_offset_regression <- pi_data %>% 
