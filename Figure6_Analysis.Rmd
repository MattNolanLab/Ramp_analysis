...
```{r}
spatial_firing <- spatial_firing %>%
  mutate(predict_mean_p = map(Rates_averaged_rewarded_b, predict_homebound)) 
...
#cue_data <- data %>% filter(cue_group == "Positive-cue" | cue_group == "Negative-cue")
...
```{r}
df <- tibble(Position = rep(1:200, times=nrow(pi_data)), Rates = unlist(pi_data$Rates_averaged_nb), Outbound_beaconed = rep(pi_data$lm_group_p, each=200), Homebound_beaconed = rep(pi_data$lm_group_p_h, each=200), reset_group = rep(as.character(pi_data$reset_group_p), each=200), predict_mean = rep(as.numeric(pi_data$predict_mean), each=200))
...
cue_data <- subset(data,cue_group == "Positive-cue" | cue_group == "Negative-cue")

```

# filter for negative > negative & positive > positive
```{r}
negneg_pineurons <-subset(pi_data, lm_group_p_o == "Negative" & lm_group_p_h == "Negative")
posos_pineurons <-subset(pi_data, lm_group_p_o == "Positive" & lm_group_p_h == "Positive")
negpos_pineurons<-subset(pi_data, lm_group_p_o == "Negative" & lm_group_p_h == "Positive")
posneg_pineurons<-subset(pi_data, lm_group_p_o == "Positive" & lm_group_p_h == "Negative")
...
p <- sankeyNetwork(Links = links, Nodes = nodes,
              ...
              Value = "value", NodeID = "name", Linkcue_group='source',
...
pi_positive <-subset(pi_data, lm_group_p_o == "Positive")
pi_negative <-subset(pi_data, lm_group_p_o == "Negative")
pi_none <-subset(pi_data, lm_group_p_o == "None")
pi_ramp <-subset(pi_data, lm_group_p_o == "Positive" | lm_group_p_o == "Negative")
