...
```{r}
mean_SEM_plots_comp <- function(df){
  plot <- mean_SEM_plots(df)
  
  plot +
    geom_ribbon(aes(x=Position, y=mean_c, ymin = mean_c - sd_c, ymax = mean_c + sd_c), fill = "grey70", alpha=0.5) +
    geom_line(aes(y=mean_c, x=Position), color = "Red") 
}
```
...
(spatial_firing %>%
  ...
  dplyr::summarise(mean_b = mean(Rates, na.rm = TRUE),
                   sd_b = std.error(Rates, na.rm = TRUE),
                   mean_c = mean(Rates_c, na.rm = TRUE),
                   sd_c = std.error(Rates_c, na.rm = TRUE)) %>%
