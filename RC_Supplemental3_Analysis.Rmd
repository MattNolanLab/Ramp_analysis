...
position_h = c(80:140)
Position = c(-29:169)
rates = unlist(unlist(spatial_firing_example$normalised_rates[1]))
rates_o <- rates[30:90]
rates_h <- rates[110:170]

ggplot() +
  geom_point(aes(x=position, y=rates), color = "grey", alpha = 0.5) +
  geom_point(aes(x=position_o, y=rates_o)) +
  #geom_smooth(aes(x=position_o, y=rates_o), method = "lm", se = FALSE, color ="red") +
  geom_point(aes(x=position_h, y=rates_h)) +
  #geom_smooth(aes(x=position_h, y=rates_h), method = "lm", se = FALSE, color ="red") +
  theme_classic() +
 ...
 ggsave(file = "plots/ExampleZ-score_Shuffled.png",  width = 4.5, height = 3.5) 
