...
```{r}
mean_SEM_plots_BvsNB <- function(df, group_b, group_nb, x_start = -10, x_end = 70){
  df <- df %>%
 ...
 df$Position <- rep(-29:170, times = numcells)
 ...
 ggplot(data=df) +
  annotate("rect", xmin=-10, xmax=0, ymin=5,ymax=Inf, alpha=0.2, fill="Grey60") +
  annotate("rect", xmin=60, xmax=70, ymin=5,ymax=Inf, alpha=0.2, fill="Chartreuse4") +
  ...
  #nnotate("rect", xmin=-10, xmax=70, ymin=3.5,ymax=4.5, alpha=0.2, fill="Chartreuse4") +
  theme_classic() +
...
if (save_figures == 1) {
  ggsave(file = "plots/AverageRates_PositionCells_NegCue.png", width = 3.2, height = 2.5)
}
```
...
if (save_figures == 1) {
  ggsave(file = "plots/AverageRates_PositionCells_PosCue.png", width = 3.2, height = 2.5)
}
```
...
if (save_figures == 1) {
  ggsave(file = "plots/AverageRates_PositionCells_PosPI.png", width = 3.2, height = 2.5)
}
```
...
if (save_figures == 1) {
  ggsave(file = "plots/AverageRates_PositionCells_NegPI.png", width = 3.2, height = 2.5)
}
```
...
if (save_figures == 1) {
  ggsave(file = "plots/AverageRates_PositionCells_NegPIonly.png", width = 3.2, height = 2.5)
}
```
...
2. take only columns we need for the rest of the analysis
```{r}
spatial_firing_save <- select(position_cells, session_id, cluster_id, unique_id, o_b_mod_coefs_pos, o_b_mod_coefs_speed, o_b_mod_coefs_accel, final_model_o_b, lm_group_b) %>%
  unnest(cols = c(unique_id, o_b_mod_coefs_pos, o_b_mod_coefs_speed, o_b_mod_coefs_accel,
    final_model_o_b, lm_group_b)) %>%
  as.tibble()
...
data_coef <- tibble(session_id = rep(unlist(spatial_firing_save$unique_id), times=3), 
                    ...
                    coef_type = rep(c("P", "S", "A"),
                                    each = nrow(spatial_firing_save)),
                    lm_result = rep(as.character(spatial_firing_save$lm_group_b), times=3),
                    cue_group = rep(as.character(spatial_firing_save$cue_group_o), times=3)
)
```
