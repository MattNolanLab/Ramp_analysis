...
spatial_firing <- spatial_firing %>%
  mutate(normalised_rates_p_smoothed = map(Rates_averaged_smoothed_p, normalise_rates))
...
```{r}
comp_plot <- function(df, plot_column = "Rates_averaged_rewarded_smoothed_p", group1 = "Positive", group2 = "Positive") {
  df$plot_column <- df[[plot_column]]
  df %>% filter(lm_group_b == group1 & lm_group_b_h == group2) %>%
    select(unique_id, plot_column) %>%
    mutate(Rates = map(plot_column, unlist)) %>%
    unnest(c(unique_id, Rates)) %>%
    mutate(Pos = rep(1:200, length(unique(unique_id)))) %>%
    ggplot(aes(x = Pos, y = Rates, colour = unique_id)) +
    geom_line()
}

(arp_P <- spatial_firing_probe_only %>% filter(final_model_o_b == "P") %>%
  comp_plot("Rates_averaged_rewarded_smoothed_p", "Positive", "Positive"))

(arp_N <- spatial_firing_probe_only %>% filter(final_model_o_b == "P") %>%
  comp_plot("Rates_averaged_rewarded_smoothed_p", "Negative", "Negative"))

arp_P+facet_wrap(~unique_id)

arp_N+facet_wrap(~unique_id)
```
...
# Make the Network
(sn_plot <- sankeyNetwork(Links = data_long, Nodes = nodes,
...
if (save_figures == 1) {
  saveNetwork(sn_plot, "plots/sn_plot.html")
}
```
