...
First split data based on cue independent/cue dependent (Figure 5) as we are only interested in position neurons here
```{r}
cue_independant_neurons <- filter(spatial_firing,  == "Positive-PI" | cue_group_o == "Negative-PI")
...
Might need to add something here to save the coefficients.
...
```{r}
data_coef_2 <- tibble(session_id = rep(unlist(spatial_firing_save$unique_id), times=3), 
...
4b. Plot coefficients for cells classified in Figure 3 as having 'significant' positional coefficients in the mixed effect model, and Figure 4 as having ramp-like activity on both beaconed and non-beaconed/probe trials. 
