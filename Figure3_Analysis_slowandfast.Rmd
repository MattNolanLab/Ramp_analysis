...
spatial_firing_high <- spatial_firing_high %>%
  mutate(asr_t_sum = pmap(list(Avg_FiringRate_TryTrials, session_id, cluster_id), add_position),
...
spatial_firing <- spatial_firing %>%
  mutate(normalised_rates_try_fast = map(Avg_FiringRate_TryTrials, normalise_rates), 
...
join_average_rates <- function(hit, run, try_s,try_f,  session_id, cluster_id) {
  ...
  df <- tibble(Rates = c(unlist(hit), unlist(run), unlist(try_f), unlist(try_s)), Reward_indicator = c(rep("Rewarded", times=199), rep("Run Through", times=200), rep("Try-fast", times=200), rep("Try-slow", times=200)), Position = c(rep(-29:169), rep(-30:169), rep(-30:169), rep(-30:169)))
  return (df)
}
```
