...
```{r}
df <- tibble(Rates = as.numeric(unlist(spatial_firing_2[1,]$spike_rate_in_time)), Speed = as.numeric(unlist(spatial_firing_2[1,]$speed_rate_in_time)), Position = as.numeric(unlist(spatial_firing_2[1,]$position_rate_in_time)))
...
lm_predict <- function(df){
  new.data <- data.frame(Speed = df$Speed, Position =df$Position)
}
