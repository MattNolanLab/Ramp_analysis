...
ggplot(data=spatial_firing, aes(x = asr_b_rewarded_fit_slope,
                                y = asr_b_rewarded_fit_adj.r.squared)) + 
...
if(!"shuffle_results_p" %in% colnames(spatial_firing)) {
  spatial_firing <- spatial_firing %>%
...
```{r}
spatial_firing <- spatial_firing %>%
  mutate(shuffle_min_slope = map(shuffle_results_b, extract_min_shuffle_slopes)) %>%
  mutate(shuffle_max_slope = map(shuffle_results_b, extract_max_shuffle_slopes)) %>%
...
extract_shuffle_slopes <- function(df){
  return(unlist(df$slope))
}

extract_shuffle_r2 <- function(df){
  return(unlist(df$rsquared))
}

extract_shuffle_pval <- function(df){
  return(unlist(df$pval))
}
```
...
adu_p <- tibble(adjust_pval = p.adjust(p_vals$pvals, "BH"))
...
spatial_firing <- spatial_firing %>%
  ...
  mutate(lm_group_nb = pmap(list(shuffle_min_slope_nb, shuffle_max_slope_nb, asr_nb_rewarded_slope_o, adjust_pval_nb), compare_slopes)) %>%
