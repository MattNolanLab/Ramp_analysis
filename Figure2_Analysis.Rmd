...
shuffle_rates <- function(df, shuffles = 1000) {
  ...
  df_mod <- lm_helper(shuff_df, 30, 90)
...
if(!"shuffle_results_b_o" %in% colnames(spatial_firing)) {
  spatial_firing <- spatial_firing %>%
    mutate(shuffle_results_b_o = map2(Rates_averaged_rewarded_b, shuffles, shuffle_rates)) %>%
    mutate(shuffle_results_nb_o = map2(Rates_averaged_rewarded_nb, shuffles, shuffle_rates)) %>%
    mutate(shuffle_results_p_o = map2(Rates_averaged_rewarded_p, shuffles, shuffle_rates))
}
