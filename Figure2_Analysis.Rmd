...
if(!"shuffle_results_b_o" %in% colnames(spatial_firing)) {
  spatial_firing <- spatial_firing %>%
    mutate(shuffle_results_b_o = map(Rates_averaged_rewarded_b, shuffle_rates)) %>%
    mutate(shuffle_results_nb_o = map(Rates_averaged_rewarded_nb, shuffle_rates)) %>%
    mutate(shuffle_results_p_o = map(Rates_averaged_rewarded_p, shuffle_rates))
}
