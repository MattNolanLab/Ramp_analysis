...
add_position <- function(df) { 
  ...
  if(any(is.na(df$Rates))) {print(typeof(df$Rates))}
  df
}
```
...
spatial_firing <- spatial_firing %>%
  mutate(asr_b_rewarded = map(Rates_averaged_rewarded_b, add_position),
         asr_nb_rewarded = map(Rates_averaged_rewarded_nb, add_position),
         asr_p_rewarded = map(Rates_averaged_rewarded_p, add_position)
         )
```
...
spatial_firing <- spatial_firing %>%
  ...
  mutate(asr_b_rewarded_fit = pmap(list(asr_b_rewarded, 30, 90), lm_helper),
         asr_nb_rewarded_fit = pmap(list(asr_nb_rewarded, 30, 90), lm_helper),
         asr_p_rewarded_fit = pmap(list(asr_p_rewarded, 30, 90), lm_helper)) %>%
...
```{r}
ggplot(data=spatial_firing, aes(x = asr_b_rewarded_fit_slope,
                                y = asr_b_rewarded_fit_r.squared)) + 
...
shuffle_rates <- function(df) {
  ...
  x = x+1
  if (x == 10){ # putting 100 here for now for testing as 1000 takes v long time to run
  break
  }
  }
return(df_modified)
...
if(!"shuffle_results_b" %in% colnames(spatial_firing)) {
  spatial_firing <- spatial_firing %>%
    mutate(shuffle_results_b = map(Rates_averaged_rewarded_b, shuffle_rates)) %>%
...
```{r}
spatial_firing <- spatial_firing %>%
  mutate(shuffle_min_slope_b = map_dbl(shuffle_results_b, extract_min_shuffle_slopes),
         shuffle_max_slope_b = map_dbl(shuffle_results_b, extract_max_shuffle_slopes),
         shuffle_min_slope_nb = map_dbl(shuffle_results_nb, extract_min_shuffle_slopes),
         shuffle_max_slope_nb = map_dbl(shuffle_results_nb, extract_max_shuffle_slopes),
         shuffle_min_slope_p = map_dbl(shuffle_results_p, extract_min_shuffle_slopes),
         shuffle_max_slope_p = map_dbl(shuffle_results_p, extract_max_shuffle_slopes))
...
1. Eextract shuffle results (slopes and r2 for each shuffle)

2. run on all neurons
```{r}
spatial_firing <- spatial_firing %>%
  select(-contains('shuffle_results_b_')) %>%
  select(-contains('shuffle_results_nb_')) %>%
  select(-contains('shuffle_results_p_')) %>%
  unnest_wider(shuffle_results_b, names_sep = "_", names_repair = "universal") %>%
  unnest_wider(shuffle_results_nb, names_sep = "_", names_repair = "universal") %>%
  unnest_wider(shuffle_results_p, names_sep = "_", names_repair = "universal")
```
...
```{r}
p_vals_b <- tibble(pvals = spatial_firing$asr_b_rewarded_fit_p.value)
adu_p_b <- tibble(adjust_pval_b = p.adjust(p_vals_b$pvals, "BH"))
...
```{r}
p_vals_nb <- tibble(pvals = spatial_firing$asr_nb_rewarded_fit_p.value)
adu_p_nb <- tibble(adjust_pval_nb = p.adjust(p_vals_nb$pvals, "BH"))
...
```{r}
p_vals_p <- tibble(pvals = spatial_firing$asr_p_rewarded_fit_p.value)
adu_p_p <- tibble(adjust_pval_p = p.adjust(p_vals_p$pvals, "BH"))
...
compare_slopes <- function(min_slope = 1, max_slope = 1, slope = 1, pval = 1){
  if (pval > 0.01) {
    return( "Unclassified" )
...
spatial_firing <- spatial_firing %>%
  mutate(lm_group_b = pmap(list(shuffle_min_slope_b, shuffle_max_slope_b, asr_b_rewarded_fit_slope, adjust_pval_b), compare_slopes),
         lm_group_nb = pmap(list(shuffle_min_slope_nb, shuffle_max_slope_nb, asr_nb_rewarded_fit_slope, adjust_pval_nb), compare_slopes),
         lm_group_p = pmap(list(shuffle_min_slope_p, shuffle_max_slope_p, asr_p_rewarded_fit_slope, adjust_pval_p), compare_slopes))
...
ggplot(data=shuff_slopes, aes(x = slopes, y = r2), color="grey32", fill="grey32") + 
    ...
    geom_point(alpha=.4) + 
    geom_point(data=spatial_firing, aes(x = asr_b_rewarded_slope_o, y = asr_b_rewarded_r2_o, color=factor(unlist(lm_group)))) +
...
ggplot(data=spatial_firing, aes(x= asr_b_rewarded_slope_o, fill=factor(unlist(lm_group), level = level_order))) + #fill=factor(unlist(lm_group))
ggplot(data=spatial_firing, aes(x= asr_b_rewarded_slope_o, fill=factor(unlist(lm_group), level = level_order))) + #fill=factor(unlist(slope_criteria_pval))
...
ggplot(data=spatial_firing, aes(x= asr_b_rewarded_r2_o, fill=factor(unlist(lm_group), level = level_order))) + #fill=factor(unlist(lm_group))
...
lm_result <- tibble(result = as.character(start_ramps$lm_group))
...
names(Var1) <- c("Negative", "Unxlassified", "Positive")
...
```{r}
saveRDS(spatial_firing, file="Fig2Results.Rda")
