...
3. Plot coefficients of model (slope and r2 value) for each neuron (just for visualizing)
```{r}
...
ggplot(proportions_mixed_ramps, aes(x= ramp_type, y = perc, fill=factor(ramp_id))) +
  ...
  theme(axis.text.x = element_text(size=19),
        ...
        axis.title.y = element_text(margin = margin(t = 0, r = 20, b = 0, l = 0)))
if (save_figures == 1) {
  ggsave(file = "plots/Outbound_ramp_proportions.png", width = 3, height = 5.5)
}
```
...
ggplot(data=spatial_firing, aes(x= asr_b_o_rewarded_fit_slope, fill=factor(unlist(lm_group_b), level = level_order))) +
  ...
  theme(axis.text.x = element_text(size=16),
        ...
        axis.title.y = element_text(margin = margin(t = 0, r = 20, b = 0, l = 0)))
if (save_figures == 1) {
  ggsave(file = "plots/Outbound_slope_histogram.png", width = 4, height = 2)
}
```
...
ggplot(data=spatial_firing, aes(x= asr_b_o_rewarded_fit_r.squared, fill=factor(unlist(lm_group_b), level = level_order))) + 
  ...
  theme(axis.text.x = element_text(size=16),
        ...
        axis.title.y = element_text(margin = margin(t = 0, r = 20, b = 0, l = 0)))
if (save_figures == 1) {
  ggsave(file = "plots/Outbound_rsquared_histogram.png", width = 4, height = 2)
}
```
...
_note this is not used in a figure_
...
if (save_figures == 1) {
  save_pheatmap_png(myheatmap, "plots/my_heatmap_all.png")
}
```
