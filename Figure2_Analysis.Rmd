...
spatial_firing <- spatial_firing %>%
  ...
  mutate(asr_b_rewarded_fit_o = pmap(list(asr_b_rewarded, 30, 90), lm_helper),
         asr_nb_rewarded_fit_o = pmap(list(asr_nb_rewarded, 30, 90), lm_helper),
         asr_p_rewarded_fit_o = pmap(list(asr_p_rewarded, 30, 90), lm_helper)) %>%
  unnest_wider(asr_b_rewarded_fit_o, names_sep = "_", names_repair = "universal") %>%
  unnest_wider(asr_nb_rewarded_fit_o, names_sep = "_", names_repair = "universal") %>%
  unnest_wider(asr_p_rewarded_fit_o, names_sep = "_", names_repair = "universal")
```

3. Plot coefficients of model (slope and r2 value) for each neuron
```{r}
ggplot(data=spatial_firing, aes(x = asr_b_rewarded_fit_o_slope,
                                y = asr_b_rewarded_fit_o_r.squared)) +
...
3. do the same for probe trials (dont run unless youve ran the shuffled for probe trials..)
```{r}
...
spatial_firing <- spatial_firing %>%
  mutate(
    ...
    lm_group_nb = pmap(
      list(
        shuffle_min_slope_nb,
        shuffle_max_slope_nb,
        asr_nb_rewarded_fit_slope,
        adjust_pval_nb
      ),
      compare_slopes
    ),
...
```{r}
shuff_slopes <- tibble(slopes = unlist(spatial_firing$shuffle_results_b_slope), r2 = unlist(spatial_firing$shuffle_results_b_r.squared))
```
...
ggplot(data=shuff_slopes, aes(x = slopes, y = r2), color="grey32", fill="grey32") + 
    ...
    geom_point(alpha=.4) + 
    geom_point(data=spatial_firing, aes(x = asr_b_rewarded_fit_slope, y = asr_b_rewarded_fit_r.squared, color=factor(unlist(lm_group_b)))) +
...
ggplot(data=spatial_firing, aes(x= asr_b_rewarded_fit_slope, fill=factor(unlist(lm_group_b), level = level_order))) + #fill=factor(unlist(slope_criteria_pval))
...
ggplot(data=spatial_firing, aes(x= asr_b_rewarded_fit_r.squared, fill=factor(unlist(lm_group_b), level = level_order))) + #fill=factor(unlist(lm_group))
...
```{r}
start_ramps<-spatial_firing[order(spatial_firing$asr_b_rewarded_fit_slope),]
