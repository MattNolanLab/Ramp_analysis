...
Remove neurons in visual cortex

```{r}
spatial_firing <- subset(spatial_firing, brain_region != "V1" )
```
...
ggsave(file = "plots/lm_allcells.png", width = 4, height = 5)
```
,
...
ggplot(proportions_mixed_ramps, aes(x= ramp_type, y = perc, fill=factor(ramp_id))) +
  ...
  theme(axis.text.x = element_text(size=19),
        ...
        axis.title.y = element_text(margin = margin(t = 0, r = 20, b = 0, l = 0))) +
ggsave(file = "plots/Outbound_ramp_proportions.png", width = 3, height = 6)
```
...
ggplot(data=spatial_firing, aes(x= asr_b_o_rewarded_fit_slope, fill=factor(unlist(lm_group_b), level = level_order))) + #fill=factor(unlist(lm_group_b))
  ...
  theme(axis.text.x = element_text(size=16),
        ...
        axis.title.y = element_text(margin = margin(t = 0, r = 20, b = 0, l = 0))) +
ggsave(file = "plots/Outbound_slope_histogram.png", width = 4.5, height = 2)
...
ggplot(data=shuff_slopes, aes(x = slopes)) +
    ...
    theme(axis.text.x = element_text(size=16),
          ...
          legend.text=element_text(size=16),
          axis.title.y = element_text(margin = margin(t = 0, r = 20, b = 0, l = 0)))
ggsave(file = "plots/Outbound_slope_histogram_shuffled.png", width = 4.5, height = 2)
```
...
ggplot(data=spatial_firing, aes(x= asr_b_o_rewarded_fit_r.squared, fill=factor(unlist(lm_group_b), level = level_order))) + #fill=factor(unlist(lm_group_b))
  ...
  theme(axis.text.x = element_text(size=16),
        ...
        legend.text=element_text(size=16),
        axis.title.y = element_text(margin = margin(t = 0, r = 20, b = 0, l = 0))) +
ggsave(file = "plots/Outbound_rsquared_histogram.png", width = 4.5, height = 2)
```
...
ggplot(data=shuff_slopes, aes(x = r2)) +
    ...
    theme(axis.text.x = element_text(size=16),
          ...
          legend.text=element_text(size=16),
          axis.title.y = element_text(margin = margin(t = 0, r = 20, b = 0, l = 0)))
ggsave(file = "plots/Outbound_rsquared_histogram_shuffled.png", width = 4.5, height = 2)
```
...
ggplot(data=shuff_slopes, aes(x = slopes, y = r2)) +
    ...
    theme(axis.text.x = element_text(size=12),
          ...
          legend.text=element_text(size=12),
          axis.title.y = element_text(margin = margin(t = 0, r = 20, b = 0, l = 0)))
ggsave(file = "plots/Outbound_coefficient_scatter_shuffled.png", width = 3, height = 3)
...
```{r}
spatial_firing <- subset(spatial_firing, Mouse != "1124")
```
...
mydf <- data.frame(row.names = paste("neuron", 1:max(start_ramps$new_cluster_id), sep="_"), region = brain_region, catagory = lm_result, ramp_score=ramp_result)
...
Var2        <- c("coral2", "deepskyblue2", "deepskyblue2", "blueviolet" )
names(Var2) <- c("PS", "UN", "RH", "MEC")

anno_col <- list(region = Var2, result = Var1, ramp_score = brewer.pal(11,"RdBu"))
...
save_pheatmap_png(myheatmap, "my_heatmap_all2.png")
```
