...
if(!"shuffle_results" %in% colnames(spatial_firing)) {
  spatial_firing <- spatial_firing %>%
...
spatial_firing <- spatial_firing %>%
  mutate(shuffle_min_slope = map(shuffle_results, extract_min_shuffle_slopes),
         shuffle_max_slope = map(shuffle_results, extract_max_shuffle_slopes))
...
spatial_firing <- spatial_firing %>%
  mutate(shuffle_slopes = map(shuffle_results, extract_shuffle_slopes),
         shuffle_rsquared = map(shuffle_results, extract_shuffle_r2),
         shuffle_pval = map(shuffle_results, extract_shuffle_pval))
...
MN: What happens here if columns are not identical? Which one is kept? Seems dangerous.
```{r}
