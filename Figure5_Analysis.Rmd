...
### First, we need to classify neurons based on their ramp activity in the homebound region, similar to that done in figure 2 analysis in the outbound region. 

# Run linear model to examine relationship between firing rate and position
_note: we are only interested in the homebound region of the track (110 - 170 cm)_
...
Now, we want to find out if within pospos and negneg groups - do their firing rates reset or continue across the reward region?

To do this, we will predict the firing rate on the homebound zone from the activity in the outbound. Then find the difference between the predicted and real data to determine if a cells firing rate has reset or continued across the reward zone. 

First, we want to normalise firing rates. 
...
2. Function to calculate mean difference in firing rate between real and predicted data
```{r}
...
_note: arbitary threshold chosen_

```{r}
classify_reset_group <- function(df, id){
  if (is.na(df) ) {
    return( "None" )
    ...
    return( "Continuous" ) 
  } else if( (df > -0.8 & df < 0.8) & (id == "pospos" | id == "negneg")) {
    return( "Continuous" )
  } else if( (df <= -0.8 | df >= 0.8) & (id == "pospos" | id == "negneg")) {
    return( "Reset" ) 
    } else {
...
```{r}
spatial_firing <- spatial_firing %>%
  mutate(reset_group = map2(predict_mean, predict_id, classify_reset_group))

```

3. Calculate number of neurons that are in each group as classified above _for manuscript_
```{r}
