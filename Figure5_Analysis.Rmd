...
```{r}
mark_track_category_reset <- function(outbound, homebound, reset){
  if (is.na(outbound) ) {
    return( "None" )  
  } else if( (outbound == "Positive") & (homebound == "Negative")) {
    return( "+ -" ) 
...
```{r}
spatial_firing <- spatial_firing %>%
  mutate(track_reset_group = pmap(list(lm_group_b, lm_group_b_h, reset_group), mark_track_category_reset)) %>%
  mutate(track_reset_group_nb = pmap(list(lm_group_nb, lm_group_nb_h, reset_group_nb), mark_track_category_reset)) 
...
mean_SEM_plots <- function(df){
  ...
  geom_ribbon(aes(x=Position, y=mean_r, ymin = mean_r - sem_r, ymax = mean_r + sem_r), fill = "grey70", alpha=0.5) +
