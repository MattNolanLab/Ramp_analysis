...
1. Use function shuffle_rates define in Figure 2.
...
shuffles <- 10 # Default 1000 but smaller values for testing.

if(!"shuffle_results_b_o" %in% colnames(spatial_firing)) {
  spatial_firing <- spatial_firing %>%
    mutate(shuffle_results_b_h = map2(Rates_averaged_rewarded_b, shuffles, shuffle_rates)) %>%
    mutate(shuffle_results_nb_h = map2(Rates_averaged_rewarded_nb, shuffles, shuffle_rates)) %>%
    mutate(shuffle_results_p_h = map2(Rates_averaged_rewarded_p, shuffles, shuffle_rates))
}
...
```{r}
spatial_firing <- spatial_firing %>%
  mutate(shuffle_min_slope_b_h = map_dbl(shuffle_results_b_h, extract_min_shuffle_slopes),
         shuffle_max_slope_b_h = map_dbl(shuffle_results_b_h, extract_max_shuffle_slopes),
         shuffle_min_slope_nb_h = map_dbl(shuffle_results_nb_h, extract_min_shuffle_slopes),
         shuffle_max_slope_nb_h = map_dbl(shuffle_results_nb_h, extract_max_shuffle_slopes),
         shuffle_min_slope_p_h = map_dbl(shuffle_results_p_h, extract_min_shuffle_slopes),
         shuffle_max_slope_p_h = map_dbl(shuffle_results_p_h, extract_max_shuffle_slopes))
...
```{r}
spatial_firing <- spatial_firing %>%
  select(-contains('shuffle_results_b_h_')) %>%
  select(-contains('shuffle_results_nb_h_')) %>%
  select(-contains('shuffle_results_p_h_')) %>%
...
spatial_firing <- spatial_firing %>%
  mutate(
    lm_group_b_h = pmap(
      list(
        shuffle_min_slope_b_h,
        shuffle_max_slope_b_h,
    ...
    lm_group_b_nb_h = pmap(
      list(
        shuffle_min_slope_nb_h,
        shuffle_max_slope_nb_h,
    ...
    lm_group_b_p_h = pmap(
      list(
        shuffle_min_slope_p_h,
        shuffle_max_slope_p_h,
...
pospos <-nrow(subset(cueindependant_neurons, track_catagory == "pospos"))/nrow(cueindependant_neurons)*100
posneg <-nrow(subset(cueindependant_neurons, track_catagory == "posneg"))/nrow(cueindependant_neurons)*100
negneg <-nrow(subset(cueindependant_neurons, track_catagory == "negneg"))/nrow(cueindependant_neurons)*100
negpos <-nrow(subset(cueindependant_neurons, track_catagory == "negpos"))/nrow(cueindependant_neurons)*100

# positive homebound slopes
pospos_num <-nrow(subset(cueindependant_neurons, track_catagory == "pospos"))
posneg_num <-nrow(subset(cueindependant_neurons, track_catagory == "posneg"))
negneg_num <-nrow(subset(cueindependant_neurons, track_catagory == "negneg"))
negpos_num <-nrow(subset(cueindependant_neurons, track_catagory == "negpos"))
...
spatial_firing <- spatial_firing %>%
  ...
  mutate(predict_params = map(Rates_averaged_rewarded_b, predict_homebound)) %>%
...
```{r}
spatial_firing <- spatial_firing %>%
  mutate(reset_group = map2(predict_mean, track_catagory, mark_reset_group))
...
```{r}
ggplot(data=subset(position_neurons, lm_group_b == "Positive"), aes(x = unlist(predict_mean), fill=as.factor(unlist(lm_group_b)))) +
...
ggplot(data=subset(position_neurons,lm_group_b == "Negative"), aes(x = unlist(predict_mean), fill=as.factor(unlist(lm_group_b)))) +
  coord_cartesian(xlim=c(-5,5), y=c(0,0.7)) +
  geom_histogram(aes(y=..density..), alpha=0.5) +
...
```{r}
df <- tibble(Position = rep(1:200, times=nrow(pi_data)), Rates = unlist(pi_data$Rates_averaged_rewarded_b), Outbound_beaconed = rep(pi_data$lm_group_b, each=200), Homebound_beaconed = rep(pi_data$lm_group_b_h, each=200), group = rep(as.character(pi_data$reset_group), each=200), predict_mean = rep(as.numeric(pi_data$predict_mean), each=200))
