...
spatial_firing <- spatial_firing %>%
  ...
  mutate(track_catagory_nb = map2(lm_group_nb, lm_group_nb_h, mark_track_catagory))
...
```{r}
mark_track_catagory <- function(outbound, homebound, reset){
  if (is.na(outbound) ) {
    return( "None" )  
  } else if( outbound == "Positive" & homebound == "Negative") {
    return( "+ -" ) 
...
```{r}
spatial_firing <- spatial_firing %>%
  mutate(track_reset_group = pmap(list(lm_group_b, lm_group_b_h, reset_group), mark_track_catagory)) %>%
  mutate(track_reset_group_nb = pmap(list(lm_group_nb, lm_group_nb_h, reset_group_nb), mark_track_catagory))
...
```{r}
position_neurons <- subset(spatial_firing, cue_group_o_o == "Positive-PI" | cue_group_o_o == "Negative-PI")
```
