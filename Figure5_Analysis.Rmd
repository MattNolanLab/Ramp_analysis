...
spatial_firing <- spatial_firing %>%
  mutate(lm_group_p = pmap(list(shuffle_min_slope_h, shuffle_max_slope_h, asr_b_rewarded_slope_h, adjust_pval_h), compare_slopes)) #%>%
...
```{r}
spatial_firing <- spatial_firing %>%
  mutate( predict_id = map2(lm_group, lm_group_p, mark_predict_group))
