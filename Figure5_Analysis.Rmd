...
author: "SHarry Clark, arah Tennant, Matt Nolan"
date: "20/10/2021"
output: html_document
---
...
ggsave(file = "plots/OFAnalysis_proportionsBOX.png", width = 6.5, height = 5.5)
...
ggplot(data=subset(all_cells_percent, ramp_type != "NULL"), aes(x=classifier2, y=percent,fill = factor(ramp_type))) + 
  ...
  theme(axis.text.x = element_text(size=18),
        ...
        legend.text=element_text(size=18), 
        axis.title.y = element_text(margin = margin(t = 0, r = 20, b = 0, l = 0))) 
ggsave(file = "plots/OFAnalysis_proportionsBOX_22.png", width = 6.5, height = 5.5)
...
spatial_firing %>% filter(cohort == 7) %>%
  ...
  mutate(speed_score = unlist(speed_score),
         ...
         ramp_type = unlist(ramp_type)) %>%
  ggplot(aes(x=ramp_type, y=speed_score, fill=ramp_type)) + 
  geom_jitter() +
  geom_violin(alpha = 0.5) +
  geom_hline(yintercept=0, linetype="dashed", color = "black") +
  labs(x = "", y = "Speed score") +
  scale_fill_manual(values=c("#F04E4E","#19D3D6","#FF9719")) +
  theme_classic() +
  theme(text = element_text(size=20),legend.position="none")

ggsave(file = "plots/speed_score_by_ramp_type.png", width = 6, height = 3.5)
...
TukeyHSD(aov(speed_score ~ ramp_type, speed_by_ramp))
...
speed_props <- spatial_firing %>%
  ...
  mutate(speed_cell = unlist(speed_cell),
    ...
    summarise(n = n()) %>%
  group_by(ramp_type, joined_id) %>%
  mutate(proportion = n / sum(n),
         percent = proportion*100)

# filter(speed_props, joined_id == "M3_7")$proportion

speed_props %>% 
  filter(speed_cell == 1) %>%
    ggplot(aes(ramp_type, proportion)) +
  geom_boxplot() +
  labs(x = "", y = "Speed cells") +
  theme_classic()+
...
spatial_firing %>% filter(cohort == 7) %>%
  ...
  theme_classic() +
  scale_colour_manual(values=c("#19D3D6","#FF9719")) +
  theme(text = element_text(size=20), legend.position = "none") +
  geom_smooth(method='lm', formula= y~x)
ggsave(file = "plots/speed_score_vs_speed_coef_by_ramp_type.png", width = 6, height = 3.5)
...
Compare speed scores with mean firing rates
In the Hippocampus high speed score cells have high firing rates (Figure 5 of Gois and Tort, 2018).
```{r}
spatial_firing %>% filter(cohort == 7) %>%
  select(speed_score, joined_id, ramp_type, o_b_mod_coefs_speed, mean_firing_rate_of) %>%
  #filter(ramp_type != "None") %>%
  mutate(speed_score = unlist(speed_score),
         joined_id = unlist(joined_id),
         ramp_type = unlist(ramp_type)) %>%
  ggplot(aes(speed_score, unlist(mean_firing_rate_of), colour = ramp_type)) +
  geom_point() +
  geom_hline(yintercept=10, linetype='dotted') +
  labs(x = "Speed score", y = "Firing rate (Hz)") +
  theme_classic() +
  scale_colour_manual(values=c("#F04E4E","#19D3D6","#FF9719")) +
  theme(text = element_text(size=20),
        legend.title=element_blank())
ggsave(file = "plots/speed_score_vs_firing_rate_by_ramp_type.png", width = 6, height = 3.5)
