...
shuffle_rates <- function(df, startbin, endbin, shuffles = 4) {
...
```{r}
spatial_firing <- spatial_firing %>%
  mutate(normalised_rates = map2(Rates_averaged_rewarded_b, Rates_averaged_rewarded_nb,normalise_rates_beaconed)) %>%
  mutate(normalised_rates_nb = map2(Rates_averaged_rewarded_b,Rates_averaged_rewarded_nb, normalise_rates_probe)) %>%
  mutate(normalised_rates_p = map2(Rates_averaged_rewarded_b,Rates_averaged_rewarded_p, normalise_rates_probe)) 
...
Function to plot mean and SEM of firing rate as a function of position.
```{r}
mean_SEM_plots2 <- function(df, colour1 = "blue"){

  ggplot(data=df) +
  annotate("rect", xmin=0, xmax=30, ymin=-2,ymax=Inf, alpha=0.2, fill="Grey60") +
  annotate("rect", xmin=170, xmax=200, ymin=-2,ymax=Inf, alpha=0.2, fill="Grey60") +
  annotate("rect", xmin=90, xmax=110, ymin=-2,ymax=Inf, alpha=0.2, fill="Chartreuse4") +
  #geom_ribbon(aes(x=Position, y=Rates, ymin = Rates - sem_r, ymax = Rates + sem_r), fill = colour1, alpha=0.2) +
  geom_line(aes(y=Rates, x=Position), color = colour1) +
  facet_wrap(~session_id + cluster, scales = "free")+
  theme_classic() +
...
mean_SEM_plots2(df_pos_cue, colour1 = "black")

if (save_figures == 1) {
 ggsave(file = "plots/pospos_mean_Hz_percell2.png", width = 30.6, height = 30.9) 
}
```
