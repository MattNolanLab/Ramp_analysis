...
if(shuffle_type=="circular") {
  spatial_firing_circ_2 <- local_circ_shuffles_2(spatial_firing, cs_path)
  
  spatial_firing <- spatial_firing %>%
  select(-contains('shuffle_results_b_o')) %>%
  select(-contains('shuffle_results_nb_o')) %>%
  select(-contains('shuffle_results_pb_o')) %>%
  mutate(shuffle_results_b_o = spatial_firing_circ_2$shuffle_results_b_o,
         shuffle_results_nb_o = spatial_firing_circ_2$shuffled_results_nb_o,
         shuffle_results_p_o = spatial_firing_circ_2$shuffled_results_p_o)
