...
if(shuffle_type=="circular") {
  cs_path = "data_in/all_mice_concatenated_shuffle_data_rewarded.feather"
  spatial_firing_circ <- local_circ_shuffles_2(spatial_firing, cs_path)
...
# if(shuffle_type=="circular") {
#   spatial_firing_circ <- load_circ_shuffles(spatial_firing, cs_path)
#   shuffles <- 1000 # this should be equivalent to the n_shuffles per cell in the .feather file
# }
# 
# spatial_firing <- spatial_firing %>%
#   sjmisc::replace_columns(spatial_firing_circ) # Could replace this with select(-contains('col_name')) and then cbind.
...
spatial_firing <- spatial_firing %>%
  ...
  mutate(shuffle_min_slope_b_o = map2_dbl(shuffle_results_b_o, 0.05, extract_quantile_shuffle_slopes),
         shuffle_max_slope_b_o = map2_dbl(shuffle_results_b_o, 0.95, extract_quantile_shuffle_slopes),
         shuffle_min_slope_nb_o = map2_dbl(shuffle_results_nb_o, 0.05, extract_quantile_shuffle_slopes),
         shuffle_max_slope_nb_o = map2_dbl(shuffle_results_nb_o, 0.95, extract_quantile_shuffle_slopes),
         shuffle_min_slope_p_o = map2_dbl(shuffle_results_p_o, 0.05, extract_quantile_shuffle_slopes),
         shuffle_max_slope_p_o = map2_dbl(shuffle_results_p_o, 0.95, extract_quantile_shuffle_slopes))
```
...
Put all pvalues into tibble, adjust for multiple comparisons and add to spatial_firing
```{r}
...
# Correct p-values from fits of the experiemntal data
...
Rep shouldn't be interleaved. Use each rather than times? Needs checking carefully.
```{r}
shuffles = 1000
