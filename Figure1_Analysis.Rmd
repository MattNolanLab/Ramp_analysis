...
spatial_firing <- spatial_firing %>%
  mutate(asr_b_rewarded = pmap(list(Rates2_averaged_rewarded_b, session_id, cluster_id), add_position),
         asr_nb_rewarded = pmap(list(Rates2_averaged_rewarded_nb, session_id, cluster_id), add_position),
         asr_p_rewarded = pmap(list(Rates2_averaged_rewarded_p, session_id, cluster_id), add_position)
         )
```
...
if(!"shuffle_results_b_o" %in% colnames(spatial_firing)) {
  spatial_firing <- spatial_firing %>%
    mutate(shuffle_results_b_o = pmap(list(Rates2_averaged_rewarded_b, 30, 90, shuffles), shuffle_rates)) %>%
    mutate(shuffle_results_nb_o = pmap(list(Rates2_averaged_rewarded_nb, 30, 90, shuffles), shuffle_rates)) %>%
    mutate(shuffle_results_p_o = pmap(list(Rates2_averaged_rewarded_p, 30, 90, shuffles), shuffle_rates))
}

#if (save_results == 1) {
  #saveRDS(spatial_firing, "SpatialFiring_with_1000_shuffles_of.Rda")
...
shuff_slopes <- tibble(slopes = unlist(spatial_firing$shuffle_results_b_o_slope), 
                       ...
                       min_slope = rep(spatial_firing$shuffle_min_slope_b_o, times = shuffles-1), 
                       max_slope = rep(spatial_firing$shuffle_max_slope_b_o, times = shuffles-1))
```
...
1. make function to load rates and normalise
```{r}
normalise_rates_probe <- function(df1, df2){
  df <- c(df1, df2)
  df <- tibble(Rates = unlist(df), Position = rep(1:398))
...
1. make function to load rates and normalise
```{r}
normalise_rates_beaconed <- function(df1, df2){
  df <- c(df1, df2)
  df <- tibble(Rates = unlist(df), Position = rep(1:398))
  #df <- tibble(Rates = unlist(df), Position = rep(1:400))
  x <- scale(df$Rates, center=TRUE, scale=TRUE)
...
```{r}
spatial_firing <- spatial_firing %>%
  mutate(normalised_rates = map2(Rates2_averaged_rewarded_b, Rates2_averaged_rewarded_nb,normalise_rates_beaconed)) %>%
  mutate(normalised_rates_nb = map2(Rates2_averaged_rewarded_b,Rates2_averaged_rewarded_nb, normalise_rates_probe)) %>%
  mutate(normalised_rates_p = map2(Rates2_averaged_rewarded_b,Rates2_averaged_rewarded_p, normalise_rates_probe)) 
...
df <- tibble(session_id = rep(pi_data$session_id, each=bin),
             cluster = rep(as.character(pi_data$cluster_id), each=bin),
             Position = rep(1:bin, times=nrow(pi_data)), 
             Rates = unlist(pi_data$normalised_rates), 
...
3. Subset data by group then average rates for plotting         
```{r}

df <-  
  tibble(average_stops = unlist(spatial_firing$average_stops), 
            average_stops_p = c(as.numeric(unlist(spatial_firing$average_stops_p)), rep(0, times=199)))
