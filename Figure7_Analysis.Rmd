...
Then, for this dataset, combine hit and miss trials together with an indicator of trial type (hit, try, run)

First, function that extracts data into a tibble, saves space in the function below

```{r}
extract_to_tibble <- function(df, name="hit") {
  df <-
  tibble(
...
join_rates <- function(hit, run, try, session_id, cluster_id) {
  ...
  hit_df <- extract_to_tibble(hit, " hit")
  run_df <- extract_to_tibble(run, " run")
  try_df <- extract_to_tibble(try, " try")
 
  df <- tibble(Rates = c(hit_df$Rates, 
                         run_df$Rates,
                         try_df$Rates), 
               Reward_indicator = c(as.factor(rep(1, times=nrow(hit_df))), 
                                    as.factor(rep(2, times=nrow(run_df))), 
                                    as.factor(rep(3, times=nrow(try_df)))), 
...
```{r}
spatial_firing_enoughtrials <- spatial_firing_enoughtrials %>%
...
compare_models_slope_lm <- function(df, run,try){
 ...
 fit <- lme4::lmer(Rates ~ Position * Reward_indicator + (1 + Position | Trials), data = df, na.action=na.omit)
 #fit2 <- update(fit,control=lmerControl(optCtrl=list(maxfun=2e4))) #this didnt get rid of the error
 modelAnova <- car::Anova(fit)
 
 #check what the fixed effect model matrix looks like 
 fix.formula <- Rates ~ Position * Reward_indicator + (1 + Position | Trials)
 X <- model.matrix (fix.formula, df)
 print(X)
...
```{r}
a <- spatial_firing_enoughtrials[1:10,] %>%
  mutate(avg_pval_slope = pmap(list(both_asr_b_by_trial, Rates_averaged_runthru_b, Rates_averaged_try_b), compare_models_slope_lm))
```
...
plot_slope_comparison (ramps$asr_b_o_rewarded_fit_slope, ramps$asr_b_run_fit_slope, ramps$reward_interaction_id)
...
```{r}
plot_slope_comparison (ramps$asr_b_o_rewarded_fit_slope, ramps$asr_b_try_fit_slope, ramps$reward_interaction_id)
...
First we want to seperate based on whether they have a positive or negative relationship with acceleration
