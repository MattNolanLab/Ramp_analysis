...
add_position <- function(df, session_id, cluster_id) {
  #print(length(df))
  if(length(df) <1){print("All NAs. Session: ")}
...
ggsave(file = "plots/SlopeScatter_reward_vs_run.png", width = 5, height = 5)
```
,5. Plot slope coefficient from linear model for both failed and rewarded trials
```{r}
ggplot(data=ramps, aes(x = asr_b_o_rewarded_fit_slope,
    ...
    theme(axis.text.x = element_text(size=12),
          ...
          legend.text=element_text(size=12), 
          axis.title.y = element_text(margin = margin(t = 0, r = 20, b = 0, l = 0))) 
ggsave(file = "plots/SlopeScatter_reward_vs_try.png", width = 5, height = 5)
```
...
ggsave(file = "plots/NegNeg_Continuous_failed_PI.png", width = 4.5, height = 2.5)
```
...
ggsave(file = "plots/NegNeg_Reset_failed_PI.png", width = 4.5, height = 2.5)
```
...
ggsave(file = "plots/PosPos_Continuous_failed_PI.png", width = 4.5, height = 2.5)
```
...
ggsave(file = "plots/PosPos_Reset_failed_PI.png", width = 4.5, height = 2.5)
```
...
ggsave(file = "plots/PosNeg_failed_PI.png", width = 3.5, height = 2.5)
```
4. Subset data by group then average rates for plotting          **Positive Unclassified**
```{r}
(NegUC_plot <- mean_SD_plots(position_neurons, "Negative", "Positive", 0, 200))
ggsave(file = "plots/AverageRates_NegPos_failed_PI.png", width = 3.5, height = 2.5)
```
