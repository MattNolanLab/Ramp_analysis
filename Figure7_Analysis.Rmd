...
compare_models_slope_lm <- function(df, run,try){
 ...
 fit <- lme4::lmer(Rates ~ Position * Reward_indicator + (Position||Reward_indicator) + (1|Trials), data = df, na.action = na.exclude) 
...
speed_neurons <- speed_neurons %>%
  mutate(speed_slope = map2(spikes_in_time, speed_fit)
         )
