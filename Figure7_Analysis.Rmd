...
Now we want to plot the slope coefficients for each trial type, marked by whether or not the cells are significantly modulated by trial type (hit, try, run)
...
_plots for pure acceleration encoding neurons (A) with POSITIVE or NEGATIVE slopes_
...
_plots for pure acceleration encoding neurons (A)_
...
_plots for pure speed encoding neurons (S)_
...
if (save_figures==1) {
  ggsave(file = "plots/negative_relationship_JustSpeed_splitbycoef.png", width = 4.5, height = 2.5)
}
```
...
if (save_figures==1) {
  ggsave(file = "plots/positive_relationship_JustSpeed_splitbycoef.png", width = 4.5, height = 2.5)
}
```
...
_plots for speed and acceleration encoding neurons (SA)_


Repeat above plots but for speed and acceleration encoding neurons, regardless of classification in Figure 2.

1. Split data based on speed and acceleration encoding group (Figure 3) as we are only interested in these neurons here
```{r}
all_speed_neurons <- filter(spatial_firing, final_model_o_b == "SA")
```
...
if (save_figures==1) {
  ggsave(file = "plots/negative_relationship_SpeedandAccel_splitbycoef.png", width = 4.5, height = 2.5)
}
```
...
if (save_figures==1) {
  ggsave(file = "plots/positive_relationship_SpeedandAccel_splitbycoef.png", width = 4.5, height = 2.5)
}
```
...
if (save_figures==1) {
  ggsave(file = "plots/negative_relationship_AllSpeed_splitbycoef.png", width = 4.5, height = 2.5)
}
```
...
if (save_figures==1) {
  ggsave(file = "plots/positive_relationship_AllSpeed_splitbycoef.png", width = 4.5, height = 2.5)
}
```
...
data <- tibble(session_id = rep(unlist(spatial_firing$unique_id), times=3), 
                    ...
                    cue_group_o = rep(as.character(spatial_firing$cue_group_o), times=3),
...
_plots for speed and speed and acceleration encoding encoding neurons (S, SA)_


1. extract only neurons that are subset based on speed and speed and acceleration encoding (Figure 3) (S, SA) and are ++ or -- in beaconed trials
```{r}
df <- data %>%
...
(pos_neg_coef_plot <- standard_pos_violin_plot(data_pos))
if (save_figures == 1) {
  ggsave(file = "plots/PredictProbe_fig7_pos_SandSA.png",width = 4, height = 2.5)
}
...
(pos_neg_coef_plot <- standard_neg_violin_plot(data_pos))
if (save_figures == 1) {
  ggsave(file = "plots/PredictProbe_fig7_neg_SandSA.png",width = 4, height = 2.5)
}
...
_plots for pure speed encoding encoding neurons (S)_

1. extract only neurons that are subset based on speed encoding (Figure 3) (S) and are ++ or -- in beaconed trials
```{r}
df <- data %>%
...
5. Select cells classified in Figure 2 as having positive or negative slopes (or coefficients) and plot
```{r}
data_pos <- df %>%
  subset(lm_group_b == "Positive")
  #subset(coef > 0) 
...
(pos_neg_coef_plot <- standard_pos_violin_plot(data_pos))
if (save_figures == 1) {
  ggsave(file = "plots/PredictProbe_fig7_pos_JustSpeed.png",width = 4, height = 2.5)
}
...
(pos_neg_coef_plot <- standard_neg_violin_plot(data_pos))
if (save_figures == 1) {
  ggsave(file = "plots/PredictProbe_fig7_neg_JustSpeed.png",width = 4, height = 2.5)
}
...
_plots for speed and acceleration encoding encoding neurons (SA)_

1. extract only neurons that are subset based on speed and acceleration encoding (Figure 3) and are ++ or -- in beaconed trials
```{r}
df <- data %>%
  filter(final_model_o_b == "SA") %>%
  filter(track_category == "pospos" | track_category == "negneg") 
...
(pos_neg_coef_plot <- standard_pos_violin_plot(data_pos))
if (save_figures == 1) {
  ggsave(file = "plots/PredictProbe_fig7_pos_JustSA.png",width = 4, height = 2.5)
}
...
(pos_neg_coef_plot <- standard_neg_violin_plot(data_pos))
if (save_figures == 1) {
  ggsave(file = "plots/PredictProbe_fig7_neg_JustSA.png",width = 4, height = 2.5)
}
  
```


_plots for pure acceleration encoding encoding neurons (A)_

1. extract only neurons that are subset based on acceleration encoding (Figure 3) and are ++ or -- in beaconed trials
```{r}
df <- data %>%
  filter(final_model_o_b == "A") %>%
  filter(track_category == "pospos" | track_category == "negneg") 
...
(pos_neg_coef_plot <- standard_pos_violin_plot(data_pos))
if (save_figures == 1) {
  ggsave(file = "plots/PredictProbe_fig7_pos_JustA.png",width = 4, height = 2.5)
}
...
(pos_neg_coef_plot <- standard_neg_violin_plot(data_pos))
if (save_figures == 1) {
  ggsave(file = "plots/PredictProbe_fig7_neg_JustA.png",width = 4, height = 2.5)
}
