...
data <- tibble(session_id = rep(unlist(df$unique_id), times=3), 
                    ...
                    coef = c(as.numeric(df$o_b_mod_coefs_pos),
                             as.numeric(df$o_b_mod_coefs_speed),
                             as.numeric(df$o_b_mod_coefs_accel)),
...
2. Plot bar charts 
```{r}
ggplot(data=subset(data,coef < 0), aes(y = unlist(predict), x=as.factor(unlist(type)), fill=as.factor(unlist(type)))) +
  coord_cartesian(ylim=c(-3.5,5.5)) +
  geom_violin(aes(y = unlist(predict), x=as.factor(unlist(type))), alpha=0.5) +
  stat_summary(fun.y=mean, geom="point", shape=23, size=2) +
  geom_point(alpha=0.3) +
