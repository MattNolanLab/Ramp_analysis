...
## plot average speed vs location for groups of neurons (P, S)
...
mean_speed_plots <- function(df, x_start = 30, x_end = 90){
 ...
 df <- df %>%
   ...
   dplyr::summarise(mean_b = mean(Speed*3, na.rm = TRUE),
             se_b = std.error(Speed*3, na.rm = TRUE))
 ggplot(data=df) +
  annotate("rect", xmin=0, xmax=30, ymin=5,ymax=Inf, alpha=0.2, fill="Grey60") +
  annotate("rect", xmin=170, xmax=200, ymin=5,ymax=Inf, alpha=0.2, fill="Grey60") +
  annotate("rect", xmin=90, xmax=110, ymin=5,ymax=Inf, alpha=0.2, fill="Chartreuse4") +
  geom_ribbon(aes(x=Position, 
   ...
   geom_line(aes(y=mean_b, 
                 x=Position, 
                 color=as.factor(Indicator), fill=as.factor(Indicator)), alpha=0.5, n= 100, span=0.2) +
   scale_fill_manual(values=c("black", "red", "blue")) +
   scale_color_manual(values=c("black", "red", "blue")) +
   theme_classic() +
...
```{r}
spatial_firing <- spatial_firing %>%
  mutate(Speed_hist_rewarded = map(spikes_in_time_rewarded_b, sum_speed_in_reward_zone),
...
```{r}
histogram_plots <- function(df, name){
  ...
  ggplot(data=df, aes(x = as.vector(mean_speed))) +
    ...
    geom_histogram(aes(y=..density..),binwidth = 1, alpha=0.5, fill='blue') +
