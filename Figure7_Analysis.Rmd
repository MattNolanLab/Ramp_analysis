...
compare_models_slope_lm <- function(df, run,try){
 ...
 fit <- lm(Rates ~ Position * Reward_indicator + (Position||Reward_indicator), data = df, na.action = na.exclude) 
 # get the model parameters
 params <- select(glance(fit), r.squared, p.value)
 print(params)
 return (params$p.value[[1]])
}
```
...
speed_neurons <- speed_neurons %>%
  mutate(speed_slope = map2(spikes_in_time, 0, speed_fit)
         )
...
3. Subset data by group then average rates for plotting          **Negative Negative Reset**
```{r}
(NegNegR_plot <- mean_SD_plots_outbound(speed_neurons_pos,  0, 200))
...
5. Subset data by group then average rates for plotting          **Positive Positive Reset**
```{r}
(PosPosR_plot <- mean_SD_plots_outbound(speed_neurons_neg, "Negative", 0, 200))
ggsave(file = "plots/Negative_negative_relationship_waccel.png", width = 4.5, height = 2.5)
```

5. Subset data by group then average rates for plotting          **Positive Positive Reset**
```{r}
(PosPosR_plot <- mean_SD_plots_outbound(speed_neurons_neg, "Positive", 0, 200))
ggsave(file = "plots/Positive_negative_relationship_waccel.png", width = 4.5, height = 2.5)
```


5. Subset data by group then average rates for plotting          **Positive Positive Reset**
```{r}
(PosPosR_plot <- mean_SD_plots_with_reset(speed_neurons_neg, "Positive", "Positive", "Reset", 0, 200))
ggsave(file = "plots/PosPos_Reset_failed_PI_negativespeed.png", width = 4.5, height = 2.5)
```

6. Subset data by group then average rates for plotting          **Positive Unclassified**
```{r}
(PosUC_plot <- mean_SD_plots(speed_neurons_pos, "Positive", "Unclassified", 0, 200))
ggsave(file = "plots/PosNeg_failed_PI_positivespeed.png", width = 4.5, height = 2.5)
```
