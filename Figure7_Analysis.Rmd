...
join_rates <- function(hit, run, try, session_id, cluster_id) {
  ...
  df <- tibble(Rates = c(hit_df$Rates, 
                         run_df$Rates,
                         try_df$Rates), 
               Reward_indicator = c(rep(1, times=nrow(hit_df)), 
                                    rep(2, times=nrow(run_df)), 
                                    rep(3, times=nrow(try_df))), 
...
compare_models_slope_lm <- function(df, run,try){
 # Check for NAs
  ...
  print(run)
  if ((length(try) < 10) | (length(run)) <10) { 
    return(NA)
  }
 df <- df %>%
 filter(Position >= 30, Position <= 90)
 fit <- lme4::lmer(Rates ~ Position * Reward_indicator + (1 + Position | Trials), data = df) 
 #print(summary(fit))
 #testing
 #nrow(df)
...
```{r}
spatial_firing <- spatial_firing %>%
  mutate(avg_pval_slope = pmap(list(both_asr_b_by_trial, spikes_in_time_runthru_b, spikes_in_time_try_b), compare_models_slope_lm))

```

To test just use the first two rows
```{r}
a <- spatial_firing[1:2,] %>%
