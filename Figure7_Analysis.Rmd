...
input columns: 
Speed_mean_rewarded = rewarded beaconed trials
Speed_mean_try =  try beaconed trials
Speed_mean_runthru = run through trials
```{r}
spatial_firing <- spatial_firing %>%
...
mean_speed_plots <- function(df, x_start = 30, x_end = 90){
 ...
 df <- df %>%
   ...
   dplyr::summarise(mean_b = mean(Speed*2, na.rm = TRUE),
             se_b = std.error(Speed*2, na.rm = TRUE))
 print(df)
 ggplot(data=df) +
  annotate("rect", xmin=0, xmax=30, ymin=5,ymax=Inf, alpha=0.2, fill="Grey60") +
  annotate("rect", xmin=170, xmax=200, ymin=5,ymax=Inf, alpha=0.2, fill="Grey60") +
...
```{r}
position_neurons <- filter(spatial_firing, final_model_o_b == "P" )
```
...
5. Split data based on encoding group (Figure 3) as we are only interested in speed neurons here
```{r}
speed_neurons <- filter(spatial_firing, final_model_o_b == "S" | final_model_o_b == "A" | final_model_o_b == "SA")
```

6. Subset data by group then average rates for plotting          
```{r}
