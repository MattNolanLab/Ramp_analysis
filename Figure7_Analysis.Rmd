...
Plot distribution of significant data

1. Extract shuffled slopes and rsquared values. This is already done above.

2. Plot in scatter
```{r}
ggplot(data=significant_ramps, aes(x = asr_b_o_rewarded_fit_slope, 
    ...
    theme(axis.text.x = element_text(size=12),
          ...
          legend.text=element_text(size=12),
          axis.title.y = element_text(margin = margin(t = 0, r = 20, b = 0, l = 0)))
ggsave(file = "plots/Sigramps_coefficient_distribution.png", width = 3, height = 3)

```

3. same as above but for the nonsignificant ramps
```{r}
ggplot(data=nonsignificant_ramps, aes(x = asr_b_o_rewarded_fit_slope, 
                                   y = asr_b_o_rewarded_fit_r.squared)) +
    geom_point() +
    xlab("\nslope") +
    ylab("R2") +
    ...
    theme(axis.text.x = element_text(size=12),
          ...
          legend.text=element_text(size=12),
          axis.title.y = element_text(margin = margin(t = 0, r = 20, b = 0, l = 0)))
ggsave(file = "plots/NonSigramps_coefficient_distribution.png", width = 3, height = 3)

```
```{r}
ggplot(data=significant_ramps, aes(x = asr_b_o_rewarded_fit_slope, 
                                   y = asr_b_o_rewarded_fit_r.squared, 
                                   fill = as.factor(unlist(final_model_o_b)))) +
    geom_point() +
    xlab("\nslope") +
    ylab("R2") +
    theme_classic() +
...
spatial_firing <- spatial_firing %>%
  mutate(avg_both_asr_b = map2(Rates_averaged_rewarded_b,Rates_averaged_failed_b, join_rates))
...
```{r}
spatial_firing <- spatial_firing %>%
  mutate(pval_slope = map(avg_both_asr_b, compare_models_slope)) %>%
  mutate(pval_intercept = map(avg_both_asr_b, compare_models_intercept))
