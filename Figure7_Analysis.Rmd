...
spatial_firing <- spatial_firing %>%
  mutate(try_trials_number = map(spikes_in_time_try_b,trial_adder),
         run_trials_number = map(spikes_in_time_runthru_b,trial_adder))
```
...
```{r}
spatial_firing <- spatial_firing %>%
  mutate(both_asr_b_by_trial = pmap(list(spikes_in_time_reward_b, spikes_in_time_runthru_b, spikes_in_time_try_b, session_id, cluster_id), join_rates))
...
```{r}
spatial_firing <- spatial_firing %>%
  mutate(pval_slope = pmap(list(both_asr_b_by_trial, Rates_averaged_runthru_b, Rates_averaged_try_b), compare_models_slope_lm))
```
...
1. Split data based on cue independant/cue dependant (Figure 5) and encoding group (Figure 3) as we are only interested in position neurons here
```{r}
allposition_neurons <- filter(spatial_firing, cue_group_o == "Positive-PI" | cue_group_o == "Negative-PI", final_model_o_b == "P" | final_model_o_b == "PS" | final_model_o_b == "PA" | final_model_o_b == "PSA")
```
...
2. Subset data by group then average rates for plotting          **Negative Negative Continuous**
```{r}
(NegNegC_plot <- mean_SD_plots_with_reset(allposition_neurons, "Negative", "Negative", "Continuous",  0, 200))
ggsave(file = "plots/NegNeg_Continuous_failed_PI_allposition.png", width = 4.5, height = 2.5)
```

3. Subset data by group then average rates for plotting          **Negative Negative Reset**
```{r}
(NegNegR_plot <- mean_SD_plots_with_reset(allposition_neurons, "Negative", "Negative", "Reset",  0, 200))
ggsave(file = "plots/NegNeg_Reset_failed_PI_allposition.png", width = 4.5, height = 2.5)
```

4. Subset data by group then average rates for plotting          **Positive Positive Continuous**
```{r}
(PosPosC_plot <- mean_SD_plots_with_reset(allposition_neurons, "Positive", "Positive", "Continuous", 0, 200))
ggsave(file = "plots/PosPos_Continuous_failed_PI_allposition.png", width = 4.5, height = 2.5)
```

5. Subset data by group then average rates for plotting          **Positive Positive Reset**
```{r}
(PosPosR_plot <- mean_SD_plots_with_reset(allposition_neurons, "Positive", "Positive", "Reset", 0, 200))
ggsave(file = "plots/PosPos_Reset_failed_PI_allposition.png", width = 4.5, height = 2.5)
```
...
2. seperate based on whether they have a positive or negative relationship with acceleration
```{r}
...
mean_SD_plots_outbound <- function(df, x_start = 30, x_end = 90){
 ...
 ggplot(data=df) +
  annotate("rect", xmin=0, xmax=30, ymin=3,ymax=Inf, alpha=0.2, fill="Grey60") +
  annotate("rect", xmin=170, xmax=200, ymin=3,ymax=Inf, alpha=0.2, fill="Grey60") +
  annotate("rect", xmin=90, xmax=110, ymin=3,ymax=Inf, alpha=0.2, fill="Chartreuse4") +
  geom_ribbon(aes(x=Position, y=mean_b, ymin = mean_b - se_b, ymax = mean_b + se_b,
                  fill=factor(Reward_indicator)), alpha=0.1) +
   geom_line(aes(y=mean_b, x=Position, color=factor(Reward_indicator)), alpha=0.5) +
   scale_fill_manual(values=c("black", "red", "blue")) +
...
3. Subset data by group then average rates for plotting **Regardless of coefficient with acceleration**
```{r}
(NegNegR_plot <- mean_SD_plots_outbound(accel_neurons,  0, 200))
ggsave(file = "plots/all_relationship_waccel.png", width = 4.5, height = 2.5)
```

3. Subset data by group then average rates for plotting  **Negative coefficient with acceleration**
```{r}
(pos_plot <- mean_SD_plots_outbound(accel_neurons_neg,  0, 200))
...
```{r}
(pos_plot <- mean_SD_plots_outbound(accel_neurons_pos,  0, 200))
...
Repeat above plots but for acceleration encoding neurons, regardless of Figure 2 classification
...
3. Subset data by group then average rates for plotting  **Regardless of acceleration coefficient**
```{r}
(NegNegR_plot <- mean_SD_plots_outbound(all_accel_neurons,  0, 200))
ggsave(file = "plots/all_relationship_wallaccel.png", width = 4.5, height = 2.5)
```
...
3. Subset data by group then average rates for plotting          **Regardless of speed coefficient**
```{r}
...
3. Subset data by group then average rates for plotting          **Regardless of speed coefficient**
```{r}
(NegNegR_plot <- mean_SD_plots_outbound(all_speed_neurons,  0, 200))
ggsave(file = "plots/all_relationship_wallspeed.png", width = 4.5, height = 2.5)
```

3. Subset data by group then average rates for plotting          **Negative coefficient with speed**
```{r}
(pos_plot <- mean_SD_plots_outbound(all_speed_neurons_neg,  0, 200))
...
3. Subset data by group then average rates for plotting          **positive coefficient with speed**
```{r}
(pos_plot <- mean_SD_plots_outbound(all_speed_neurons_pos,  0, 200))
