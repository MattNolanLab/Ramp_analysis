...
```{r}
speed_neurons <- filter(spatial_firing, lm_group_b == "Positive" | lm_group_b == "Negative", final_model_o_b == "S" | final_model_o_b == "SA" | final_model_o_b == "A" )
```
...
3. Subset data by group then average rates for plotting          **Negative Negative Reset**
```{r}
(NegNegR_plot <- mean_SD_plots_with_reset(speed_neurons_pos, "Negative", "Negative", "Reset",  0, 200))
ggsave(file = "plots/NegNeg_Reset_failed_PI_positivespeed.png", width = 4.5, height = 2.5)
```

3. Subset data by group then average rates for plotting          **Negative Negative Reset**
```{r}
(NegNegR_plot <- mean_SD_plots_with_reset(speed_neurons_neg, "Negative", "Negative", "Reset",  0, 200))
ggsave(file = "plots/NegNeg_Reset_failed_PI_negativespeed.png", width = 4.5, height = 2.5)
```


5. Subset data by group then average rates for plotting          **Positive Positive Reset**
```{r}
(PosPosR_plot <- mean_SD_plots_with_reset(speed_neurons_pos, "Positive", "Positive", "Reset", 0, 200))
ggsave(file = "plots/PosPos_Reset_failed_PI_positivespeed.png", width = 4.5, height = 2.5)
```
5. Subset data by group then average rates for plotting          **Positive Positive Reset**
```{r}
