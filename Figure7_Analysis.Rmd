...
Function to return a tibble with proportions of neurons classified as significant or none-significant.
```{r}
calc_props_sig_dep <- function(reward_group, encoding_group){
  reward_group <- as.character(reward_group)
  encoding_group <- as.character(encoding_group)
  num_cellsP <- length(which(encoding_group == "P"))
  num_cellsA <- length(which(encoding_group == "A"))
  num_cellsS <- length(which(encoding_group == "S"))
  num_cellsPS <- length(which(encoding_group == "PS"))
  num_cellsPA <- length(which(encoding_group == "PA"))
  num_cellsSA <- length(which(encoding_group == "SA"))
  num_cellsPSA <- length(which(encoding_group == "PSA"))

  sig_P_num <- length(which(reward_group == "Significant" & encoding_group == "P"))
  sig_A_num <- length(which(reward_group == "Significant" & encoding_group == "A"))
  sig_S_num <- length(which(reward_group == "Significant" & encoding_group == "S"))
  sig_SA_num <- length(which(reward_group == "Significant" & encoding_group == "SA"))
  sig_PA_num <- length(which(reward_group == "Significant" & encoding_group == "PA"))
  sig_PS_num <- length(which(reward_group == "Significant" & encoding_group == "PS"))
  sig_PSA_num <- length(which(reward_group == "Significant" & encoding_group == "PSA"))
  notsig_P_num <- length(which(reward_group != "Significant" & encoding_group == "P"))
  notsig_A_num <- length(which(reward_group != "Significant" & encoding_group == "A"))
  notsig_S_num <- length(which(reward_group != "Significant" & encoding_group == "S"))
  notsig_SA_num <- length(which(reward_group != "Significant" & encoding_group == "SA"))
  notsig_PA_num <- length(which(reward_group != "Significant" & encoding_group == "PA"))
  notsig_PS_num <- length(which(reward_group != "Significant" & encoding_group == "PS"))
  notsig_PSA_num <- length(which(reward_group != "Significant" & encoding_group == "PSA"))

  sig_P <- (sig_P_num / num_cellsP) *100
  sig_A <- (sig_A_num / num_cellsA) *100
  sig_S <- (sig_S_num / num_cellsS) *100
  sig_PA <- (sig_PA_num / num_cellsPA) *100
  sig_PS <- (sig_PS_num / num_cellsPS) *100
  sig_SA <- (sig_SA_num / num_cellsSA) *100
  sig_PSA <- (sig_PSA_num / num_cellsPSA) *100
  nonsig_P <- (notsig_P_num / num_cellsP) *100
  nonsig_A <- (notsig_A_num / num_cellsA) *100
  nonsig_S <- (notsig_S_num / num_cellsS) *100
  nonsig_PA <- (notsig_PA_num / num_cellsPA) *100
  nonsig_PS <- (notsig_PS_num / num_cellsPS) *100
  nonsig_SA <- (notsig_SA_num / num_cellsSA) *100
  nonsig_PSA <- (notsig_PSA_num / num_cellsPSA) *100
  
mixed_ramps <- tibble(perc=c(sig_P,nonsig_P,sig_S, nonsig_S, sig_A, nonsig_A, sig_PS, nonsig_PS, sig_PA, nonsig_PA, sig_SA, nonsig_SA, sig_PSA, nonsig_PSA), 
...
Calculate proportions for neurons with +/- ramps on beaconed trials.
```{r}
mixed_ramps <- calc_props_sig_dep(spatial_firing$reward_interaction_id, spatial_firing$final_model_o_b)
```
...
2. Then make stacked histogram of slope values for significant dataset
```{r}
ggplot(data=subset(ramps, reward_interaction_id == "Significant" ), aes(x= asr_b_o_rewarded_fit_slope, fill="red")) +
  coord_cartesian(xlim = c(-0.6,0.6)) +
  geom_histogram(aes(y=..count../sum(..count..)), binwidth=0.01) +
  ylab("Proportion") +
  theme_classic() +
...
3. same as above but for not significant datasets
```{r}
ggplot(data=subset(ramps, reward_interaction_id == "Significant"), aes(x= asr_b_o_rewarded_fit_slope, fill="grey")) +
  coord_cartesian(xlim = c(-0.6,0.6)) +
  geom_histogram(aes(y=..count../sum(..count..)), binwidth=0.01, fill="Grey") +
  ylab("Proportion") +
  theme_classic() +
...
4. Make stacked histogram of rsquared values for significant dataset
```{r}
ggplot(data=subset(ramps, reward_interaction_id == "Significant"), aes(x= asr_b_o_rewarded_fit_r.squared, fill="red")) +
  coord_cartesian(xlim = c(0,1)) +
  geom_histogram(aes(y=..count../sum(..count..)), binwidth=0.01) +
  ylab("Proportion") +
  theme_classic() +
...
```{r}
ggplot(data=subset(ramps, reward_interaction_id == "Significant"), aes(x= asr_b_o_rewarded_fit_r.squared, fill="Grey")) +
  coord_cartesian(xlim = c(0,1)) +
  geom_histogram(aes(y=..count../sum(..count..)), binwidth=0.01, fill="Grey") +
  ylab("Proportion") +
  theme_classic() +
...
mean_SD_plots <- function(df, group_b, group_nb, track_group, x_start = 30, x_end = 90){
  df <- df %>%
    ...
    select(Rates_averaged_rewarded_b, Rates_averaged_failed_b) %>%
 ...
 df <- df %>%
   group_by(Position) %>%
   dplyr::summarise(mean_b = mean(Rates_averaged_rewarded_b, na.rm = TRUE),
...
ggsave(file = "plots/AverageRates_NegNeg_Continuous_failed.png", width = 3.5, height = 2.5)
```
...
ggsave(file = "plots/AverageRates_NegNeg_Reset_failed.png", width = 3.5, height = 2.5)
```
