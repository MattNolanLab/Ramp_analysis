...
if (save_figures==1) {
  ggsave(file = "plots/negative_relationship_JustSpeed.png", width = 4.5, height = 2.5)
}
```
...
if (save_figures==1) {
  ggsave(file = "plots/positive_relationship_JustSpeed.png", width = 4.5, height = 2.5)
}
```
...
1. Split data based on speed encoding group (Figure 3) as we are only interested in these neurons here
```{r}
all_speed_neurons <- filter(spatial_firing,final_model_o_b == "A")
```
...
if (save_figures==1) {
  ggsave(file = "plots/negative_relationship_JustAccel.png", width = 4.5, height = 2.5)
}
```
...
if (save_figures==1) {
  ggsave(file = "plots/positive_relationship_JustAccel.png", width = 4.5, height = 2.5)
}
```
...
if (save_figures==1) {
  ggsave(file = "plots/negative_relationship_SpeedandAccel.png", width = 4.5, height = 2.5)
}
```
...
if (save_figures==1) {
  ggsave(file = "plots/positive_relationship_JSpeedandAccel.png", width = 4.5, height = 2.5)
}
```
...
data <- tibble(session_id = rep(unlist(spatial_firing$unique_id), times=3), 
                    ...
                    coef = c(as.numeric(spatial_firing$o_b_mod_coefs_pos),
                             as.numeric(spatial_firing$o_b_mod_coefs_speed),
                             as.numeric(spatial_firing$o_b_mod_coefs_accel)),
