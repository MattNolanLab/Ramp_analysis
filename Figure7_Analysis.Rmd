...
```{r}
spatial_firing <- spatial_firing %>%
  mutate(both_asr_b_by_trial = pmap(list(spikes_in_time_reward_allspeeds, spikes_in_time_run_allspeeds, spikes_in_time_try_allspeeds, session_id, cluster_id), join_rates))
...
```{r}
spatial_firing <- spatial_firing %>%
  mutate(pval_slope = pmap(list(both_asr_b_by_trial, Avg_FiringRate_RunTrials_allspeeds, Avg_FiringRate_TryTrials_allspeeds), compare_models_slope_lm))
```
...
```{r}
spatial_firing <- spatial_firing %>%
  mutate(asr_b_try = pmap(list(Avg_FiringRate_TryTrials_allspeeds, session_id, cluster_id), add_position),
         asr_b_run = pmap(list(Avg_FiringRate_RunTrials_allspeeds, session_id, cluster_id), add_position)
         )
```
...
```{r}
spatial_firing <- spatial_firing %>%
  mutate(avg_both_asr_b = pmap(list(Avg_FiringRate_HitTrials_allspeeds, Avg_FiringRate_RunTrials_allspeeds, Avg_FiringRate_TryTrials_allspeeds, session_id, cluster_id), join_average_rates))
```
