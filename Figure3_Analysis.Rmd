...
⎄ 
```{r}
spatial_firing[!(sapply(spatial_firing$Avg_FiringRate_TryTrials, anyNA) | sapply(spatial_firing$Avg_FiringRate_RunTrials, anyNA)),] %>% filter(lm_group_b == "Positive" | lm_group_b == "Negative") %>%
  filter(lm_group_b == "Positive", final_model_o_b == "P") %>%
  ...
  ggplot(aes(x = fct_relevel(Outcome, "Hit", "Try", "Run"), y = Slope, group = unique_id)) +
  geom_point() +
  geom_line() +
  theme_classic()
```
...
outcomes_offsets_data <- tibble(session_id = rep(unlist(spatial_firing$unique_id), times=3), 
                    ...
                    predict = c(as.numeric(spatial_firing$predict_diff_hit),
