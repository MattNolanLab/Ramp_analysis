...
Analysis to test whether trial outcome has a significant effect on firing rate slope. This isn't does't directly test the core hypotheses, as we know some neurons have speed / acceleration modulated firing, so we make it optional.

1. First combine hit and miss trials together with an indicator of trial type (hit, try, run). Uses join_rates.

2. Fit linear model to all cells and extract pval
Fitting uses a generalised linear mixed effect model with a Tweedie response distribution (compare_models_slope_glm).
Similar results are obtained using a linear mixed effect model (use the function compare_models_slope_lm instead).
* Warning *
The model fitting here takes a long time.

3. Then classify cells based on the significance of the interaction between trial outcome and position in the above model (mark_neurons_sig). 
```{r}
test_outcomes = 0
...
if (test_outcomes == 1) {
  spatial_firing <- spatial_firing %>%
    mutate(both_asr_b_by_trial = pmap(list(spikes_in_time_reward, spikes_in_time_run, spikes_in_time_try, session_id, cluster_id), join_rates),
           pval_slope = pmap(list(both_asr_b_by_trial, Avg_FiringRate_RunTrials, Avg_FiringRate_TryTrials), compare_models_slope_glm),
           reward_interaction_id = map(pval_slope, mark_neurons_sig))
...
Split data based on conjunctive position encoding (PA, PS, PSA) (Figure 2) as we are only interested in these neurons here
...
Onset data for position encoding encoding neurons (P, PA, PS, PSA)
```{r}
outcomes_offsets_data_pospos <- outcomes_offsets_data %>%
...
outcomes_offsets_data_negneg <- outcomes_offsets_data %>% 
  filter(final_model_o_b == "P" | final_model_o_b == "PA"  | final_model_o_b == "PS"  | final_model_o_b == "PSA",
...
_plots for conjunctive position encoding encoding neurons (PA, PSA, PS)_
1. extract only neurons that are subset based on speed encoding(PA, PSA, PS) (Figure 3) and are ++ or -- in beaconed trials
```{r}
outcomes_offsets_data_pospos <- outcomes_offsets_data %>%
  filter(final_model_o_b == "PA" | final_model_o_b == "PSA" | final_model_o_b == "PS",
                lm_group_b == "Positive" & lm_group_b_h == "Positive")
...
if (save_figures == 1) {
  ggsave(file = "plots/PredictProbe_pos_conjunctivePosition.png",width = 4, height = 3)
}
...
if (save_figures == 1) {
  ggsave(file = "plots/PredictProbe_neg_conjunctivePosition.png",width = 4, height = 3)
}
  
```

3. run anova on above data
```{r}
one.way <- aov(predict ~ type, data = outcomes_offsets_data_pospos)
summary(one.way)
...
4. One-sample ttest to see whether offsets are above 0
...
_plots for all Position only encoding encoding neurons (P)_
1. extract only neurons that are subset based on speed encoding (P) (Figure 3) and are ++ or -- in beaconed trials
```{r}
outcomes_offsets_data_pospos <- outcomes_offsets_data %>%
...
if (save_figures == 1) {
  ggsave(file = "plots/PredictProbe_pos_onlyPosition.png",width = 4, height = 3)
}
...
if (save_figures == 1) {
  ggsave(file = "plots/PredictProbe_neg_onlyPosition.png",width = 4, height = 3)
}
...
2. One-sample ttest to see whether offsets are above 0
...
_plots for all speed encoding encoding neurons (S, SA)_
1. extract only neurons that are subset based on speed encoding (S, SA) (Figure 3) and are ++ or -- in beaconed trials
```{r}
outcomes_offsets_data_speedpos <- outcomes_offsets_data %>%
  filter(final_model_o_b == "S" | final_model_o_b == "SA",
                lm_group_b == "Positive" & lm_group_b_h == "Positive")
outcomes_offsets_data_speedneg <- outcomes_offsets_data %>% 
  filter(final_model_o_b == "S" | final_model_o_b == "SA",
                lm_group_b == "Negative" & lm_group_b_h == "Negative")
```
...
outcomes_offsets_data_speedpos %>%
  offset_groups_violin_plot(-5, 3)
if (save_figures == 1) {
  ggsave(file = "plots/PredictProbe_pos_AllSpeed.png",width = 4, height = 3)
}
outcomes_offsets_data_speedneg %>%
  offset_groups_violin_plot(-3.5, 5.5)
 
if (save_figures == 1) {
  ggsave(file = "plots/PredictProbe_neg_AllSpeed.png",width = 4, height = 3)
}
...
Find proportion & numbers of neurons that are significant/non-significant
