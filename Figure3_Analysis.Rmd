...
We might also want to look at the proportions in cue independant/cue dependant groups (defined in Figure 4)

First split data based on cue independant/cue dependant (Figure 4) as we are only interested in path integrating neurons/cue dependant neurons seperatly here
```{r}
cue_independant_neurons <- filter(spatial_firing, cue_group_o == "Positive-PI" | cue_group_o == "Negative-PI")
cue_dependant_neurons <- filter(spatial_firing, cue_group_o == "Positive-cue" | cue_group_o == "Negative-cue")

```

1. Split data by classification defined in Figure 2
```{r}
# extracting diff models 
pos <-subset(cue_dependant_neurons, lm_group_b == "Positive")
neg <-subset(cue_dependant_neurons, lm_group_b == "Negative")
none <-subset(cue_dependant_neurons, lm_group_b == "Unclassified")
...
```{r}
spatial_firing_save <- select(spatial_firing, session_id, cluster_id, unique_id, o_b_mod_coefs_pos, o_b_mod_coefs_speed, o_b_mod_coefs_accel, final_model_o_b, lm_group_b, cue_group_o) %>%
  unnest(cols = c(unique_id, o_b_mod_coefs_pos, o_b_mod_coefs_speed, o_b_mod_coefs_accel,
    final_model_o_b, lm_group_b, cue_group_o)) %>%
  as.tibble()
...
6a. Plot coefficients for cells classified in Figure 2 as having a positive slope and in Figure 3 as having signficant fit coefficients for position only.
```{r}
...
6b. Plot coefficients for cells classified in Figure 2 as having a negative slope and in Figure 3 as having signficant fit coefficients for position only.
```{r}
...
6c. Plot coefficients for cells classified in Figure 2 as being unclassified but that have 'signficant' positional coefficients in the mixed effect model.
```{r}
...
4b. Plot coefficients for cells classified in Figure 3 as having 'signficant' positional coefficients in the mixed effect model, and Figure 4 as having ramp-like activity on both beaconed and non-beaconed/probe trials. 

```{r}
data_coef_PSA_PI_pos <- data_coef %>%
  subset(group == "P" | group == "PS" | group == "PA" | group == "PSA") %>%
...
```{r}
data_coef_PSA_PI_neg <- data_coef %>%
  subset(group == "P" | group == "PS" | group == "PA" | group == "PSA") %>%
