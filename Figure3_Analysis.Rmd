...
## ratio of coefficients in CAR model
```{r}
...
coef_ratio_pos <- function(df){
  ...
  #--> calculate ratio from model parameters
  #pos = as.numeric(prtAnova$"Pr(>Chisq)"[1])
  #speed = as.numeric(prtAnova$"Pr(>Chisq)"[2])
  #p_s_ratio = as.numeric(pos/speed)
...
```{r}
spatial_firing <- spatial_firing  %>%
  mutate(o_pos_b_ratio = map(spikes_in_time, coef_ratio_pos)) %>%
  mutate(o_speed_b_ratio = map(spikes_in_time, coef_ratio_speed)) %>%
  mutate(o_accel_b_ratio = map(spikes_in_time, coef_ratio_accel))

```

# save to csv file 
```{r}
spatial_firing_save <- subset(spatial_firing, select = c("session_id", "cluster_id", "o_pos_b_ratio", "o_speed_b_ratio","o_accel_b_ratio", "final_model_o_b", "slope_criteria_pval"))

#"predict_mse", "predict_mse_p","predict_results", "predict_results_p"
spatial_firing_save <- tibble(session_id = spatial_firing_save$session_id, cluster_id = spatial_firing_save$cluster_id, pos_stdcoef =  as.character(spatial_firing_save$o_pos_b_ratio), speed_stdcoef =  as.character(spatial_firing_save$o_speed_b_ratio), accel_stdcoef =  as.character(spatial_firing_save$o_accel_b_ratio), final_model_o_b =  as.character(spatial_firing_save$final_model_o_b), lm_result_o_rewarded_b =  as.character(spatial_firing_save$slope_criteria_pval))

write.table(spatial_firing_save, "all_results_coefficients.txt", quote=FALSE, sep="\t")
```

```{r}
data_coef <- tibble(session_id = rep(unlist(spatial_firing_save$unique_id), times=3), cluster_id = rep(spatial_firing_save$cluster_id, times=3), coef = c(as.character(spatial_firing_save$o_pos_b_ratio), as.character(spatial_firing_save$o_speed_b_ratio), as.character(spatial_firing_save$o_accel_b_ratio)), group = c(as.character(spatial_firing_save$final_model_o_b), as.character(spatial_firing_save$final_model_o_b), as.character(spatial_firing_save$final_model_o_b)), coef_type = rep(c("P", "S", "A"), each = nrow(spatial_firing_save)), lm_result = rep(as.character(spatial_firing_save$slope_criteria_pval), times=3)) 
```

```{r}
level_order <- c("P", "S", "A")

ggplot(data=data_coef, aes(x = factor(coef_type), y = as.numeric(coef))) +
  ...
  theme(axis.text.x = element_text(size=12),
        ...
        legend.text=element_text(size=12), 
        axis.title.y = element_text(margin = margin(t = 0, r = 20, b = 0, l = 0))) +
ggsave(file = "plots/CoefficientValues_allcells.png", width = 4, height = 4.5)
...
ggplot(data=data_coef, aes(x = factor(coef_type), y = as.numeric(coef))) +
  ...
  stat_summary(fun.y=mean, geom="point", shape=23, size=2) +
  coord_cartesian(ylim=c(0,1)) +
  geom_point(alpha=0.05, position = pd) +
  geom_line(position = pd, aes(group = factor(session_id)), color="black") +
  labs(y = "std coef \n", x="\n model parameter") +
  theme_classic() +
  theme(axis.text.x = element_text(size=12),
        axis.text.y = element_text(size=12),
        ...
        legend.text=element_text(size=12), 
        axis.title.y = element_text(margin = margin(t = 0, r = 20, b = 0, l = 0))) +
ggsave(file = "plots/CoefficientValues_allcells_line.png", width = 4, height = 4.5)
...
ggplot(data=data_coef_pos, aes(x = factor(coef_type), y = as.numeric(coef))) +
  ...
  theme(axis.text.x = element_text(size=12),
        ...
        legend.text=element_text(size=12), 
        axis.title.y = element_text(margin = margin(t = 0, r = 20, b = 0, l = 0))) +
ggsave(file = "plots/CoefficientValues_Position_cells_unclassfied.png", width = 4, height = 4.5)
