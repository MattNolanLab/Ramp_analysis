...
data_coef <- tibble(session_id = rep(unlist(spatial_firing_save$unique_id), times=3), 
                    ...
                    coef = c(as.character(spatial_firing_save$o_pos_b_stdcoef), as.character(spatial_firing_save$o_speed_b_stdcoef), as.character(spatial_firing_save$o_accel_b_stdcoef)), 
                    group = c(as.character(spatial_firing_save$final_model_o_b), as.character(spatial_firing_save$final_model_o_b), as.character(spatial_firing_save$final_model_o_b)), 
                    coef_type = rep(c("P", "S", "A"), each = nrow(spatial_firing_save)), lm_result = rep(as.character(spatial_firing_save$lm_group), times=3)) 
```
...
ggplot(data=data_coef_pos, aes(x = factor(coef_type), y = as.numeric(coef))) +
  ...
  theme(axis.text.x = element_text(size=12),
        ...
        legend.text=element_text(size=12), 
        axis.title.y = element_text(margin = margin(t = 0, r = 20, b = 0, l = 0))) +
ggsave(file = "plots/CoefficientValues_Position_positivecells.png", width = 4, height = 4.5)
...
ggplot(data=data_coef_neg, aes(x = factor(coef_type), y = as.numeric(coef))) +
  ...
  theme(axis.text.x = element_text(size=12),
        ...
        legend.text=element_text(size=12), 
        axis.title.y = element_text(margin = margin(t = 0, r = 20, b = 0, l = 0))) +
ggsave(file = "plots/CoefficientValues_Position_negativecells.png", width = 4, height = 4.5)
